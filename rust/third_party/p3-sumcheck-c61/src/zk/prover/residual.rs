    ...
    EF: ExtensionField<F>,
{
    ...
    {
        self.into_zk_sumcheck_with_claim_binding(
            zk_data,
            encoding,
            mmcs,
            folding_factor,
    ...
    /// C6.1 claimless variant.  The authenticated scalar is fixed outside the
    /// public transcript and is never sent to or absorbed by the challenger.
    #[allow(clippy::too_many_arguments)]
    pub fn into_zk_sumcheck_claimless<Enc, M, R, Ch>(
        self,
        zk_data: &mut ZkSumcheckData<F, EF>,
        encoding: &Enc,
        mmcs: &M,
...
mod tests {
    ...
    impl ZkEncoding<EF> for SentinelEncoding {
        ...
        fn sample_message<R: Rng>(&self, _rng: &mut R) -> Vec<EF> {
            (0..self.ell_zk).map(|idx| EF::from_u64(100 + idx as u64)).collect()
        }
    ...
    fn residual_prover_zk_handoff_replays_from_claim() {
        ...
        let gammas = prover_handoff.randomness.iter().copied().collect::<Vec<_>>();
