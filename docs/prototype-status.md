...
record and pins `4,975,525` typed leaves, but the complete GPT-2 DAG/cache
constraint census, wrapper proof, local end-to-end and hardware gates remain
pending.
...
| C6 Δ-residual inline certificate | **LEAN AMPLIFICATION + PRODUCT-CLOSURE / RUST RESIDUAL-IR / HIDDEN-U REFERENCE / EXECUTABLE PRE-BACKEND ROOFLINE / PAIRED CODEC+STATE GREEN; PRODUCTION CENSUS PENDING; HARD STOP BEFORE POD** (2026-07-29) | Hide direct `auth_corrections` and Ligero `u_vectors`; one inline native-field wrapper; fixed-capacity committed KV cache; 17 accepts plus retry reserve; no deferred settlement | The retained Q=121 response is exactly **29,176,632 B**.  The complete new-payload budget is **5,823,368 B**, while the owner cap `pi_final <=4,500,000 B` now has the stricter implemented roofline maximum **4,409,824 B**; the corresponding complete response maximum is **33,586,456 B <=35,000,000 B**.  Q=121 gives **79.472744138609166392... bits** after four `2^-128` wrapper allocations, preserving the **78.80929487391641-bit** per-certificate floor.  Exact-field review requires two complete independent repetitions of all four named events; the Δ event uses the ordinary tape plus an independent residual-only tape because same-`Delta` RLC repetition does not amplify a common root.  Each tape fits **17 accepted + 4 abort/retry** ranges and leaves **969,186** raw correlations; the conservative paired PCG setup is **76,742,930 B**, leaving **73,257,070 B** for parameters/framing under 150 MB.  The enlarged Lean build/audit is **3,257 jobs / 274 total / 38 C6**, zero `sorry`/`admit`: it proves the independent-product cardinality square, same-secret non-amplification, the two-secret residual bound, hidden-linear **>243-bit** and Δ-event **>253-bit** exact inequalities, while retaining `ProductClosure` for the nonlinear verifier node.  Rust now has canonical schema-v2 setup/state/certificate codecs plus a commit-before-challenge authenticated-value DAG with add/sub/scale, independent base-share binding and opaque QuickSilver closure; ordinary key multiplication fails construction.  The setup binds two distinct ordered tape identities and the client state, attempts, slot journal and certificate bind its digest.  Paired ranges are one indivisible value, either-coordinate overlap rejects, abort burns both, and ambiguous ACK remains byte-identical only.  C6 is **20/20 PASS**, and the complete `volta-proto` suite is **130 PASS / 1 pre-existing production-size ignore**.  Pinned paired-codec fixture lengths/digests are setup **437 B / `c3388a14...b29d75`**, genesis state **292 B / `19325552...402c51`**, and small certificate **935 B / `454a4482...5ca8c1`**; old schema-v1 magic is rejected.  The fixture first exchange is exactly **76,743,367 B** including both PCG tapes and setup framing.  The additive hidden-`u` reference fixes the production Q=121 layouts at **2^21 + 2^19 padded `Fp2` entries**, removes exactly **17,235,968 B**, binds **12,686** NTT/`ip` relations under two complete response-wide repetitions, and emits a **1,912-B** prequery frame plus one **32-B** client seed before proof bytes.  Its scaled prequery golden is **208 B / `7e6ae8da...f3e7fc`**; old `verify_multi_open` parity and NTT/`ip`/schedule/canonical tamper rejection are green.  Targeted hidden-`u` is **5/5 PASS**; full `volta-pcs` is **147 PASS / 3 pre-existing production-size ignores**.  The executable roofline fixes `s=86`, two complete packed chains at **3,609,824 B**, a hard **800,000-B** non-PCS allocation, `pi_final <=4,409,824 B` and complete response **<=33,586,456 B**; its exact arithmetic and collision-safe frontier tests are **8/8 PASS**.  The **8.380-s** kernel figure remains informative only.  This remains a witness-bearing reference, not a succinct verifier: the full GPT-2 old-schedule census, hidden-correction/cache wrapper proof, Rust end-to-end result, hardware verdict and pod contact remain pending. |
