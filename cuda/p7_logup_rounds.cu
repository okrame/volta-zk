...
#pragma omp parallel for schedule(static)
    ...
    std::vector<RoundAcc> gpu_messages(log2_n);
    auto run_gpu = [&] {
        DeviceVec4* src = &d0;
        DeviceVec4* dst = &d1;
        size_t len = n;
        for (int round = 0; round < log2_n; ++round) {
            ...
            CUDA_CHECK(cudaMemcpy(
                &gpu_messages[round], root, sizeof(RoundAcc), cudaMemcpyDeviceToHost));
        ...
        Fp2 completion[4]{};
        CUDA_CHECK(cudaMemcpy(completion, src->v[0], sizeof(completion), cudaMemcpyDeviceToHost));
    };
    ...
    for (int rep = 0; rep < gpu_reps; ++rep) {
        ...
        gpu_samples.push_back(std::chrono::duration<double>(t1 - t0).count());
    }
    std::sort(gpu_samples.begin(), gpu_samples.end());
    const double gpu_s = gpu_samples[gpu_samples.size() / 2];
    ...
    std::vector<RoundAcc> cpu_messages(log2_n);
    auto run_cpu = [&] {
        HostVec4* src = &h0;
        HostVec4* dst = &h1;
        size_t len = n;
        for (int round = 0; round < log2_n; ++round) {
            const size_t pairs = len / 2;
            cpu_messages[round] = cpu_eval(*src, s, pairs);
