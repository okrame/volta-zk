//! Goldilocks PCG expansion for VOLTA.
//!
...
//!
//! P7 phase A is retained as the historical trusted-dealer cost model. Phase B
...
const PROFILE: &str = "p7-wykw-goldilocks-regular-lpn-hardened-v3";
pub(crate) const GAMMA: Fp2 = Fp2 { c0: Fp::ZERO, c1: Fp::ONE };

#[derive(Clone, Debug, Serialize, Deserialize)]
pub struct PhaseAParams {
    pub profile: String,
    pub security_bits: u32,
    pub setup_lpn_estimated_security_bits: f64,
    pub setup_lpn_security_margin_bits: f64,
    pub extend_lpn_estimated_security_bits: f64,
    pub extend_lpn_security_margin_bits: f64,
    ...
    pub ggm_depth: u32,
    pub setup_lpn_k: usize,
    pub setup_lpn_n: usize,
    pub setup_lpn_noise_weight: usize,
    pub setup_ggm_block_size: usize,
    pub setup_ggm_depth: u32,
    pub consistency_reps: usize,
    pub noise_structure: String,
...
impl PhaseAParams {
    pub fn for_counts(sub_corrs: usize, full_corrs: usize) -> PhaseAParams {
        ...
        // The main tuple is WYKW/Wolverine, ePrint 2020/925, Section 6.1
        // Table 2. The regular noise distribution is Definition 1 in Section
        // 5.2: one uniform nonzero error at a uniform position in each
        // disjoint block. The published one-time tuple's k0=19,870 is not
        // used: Briaud--Oeygarden, ePrint 2023/176, Section 5, estimates only
        // 126.44 bits. Raising k0 to 25,000 gives 140.65 bits in the pinned
        // Code Estimators regular-LPN sweep (commit recorded below).
        let lpn_k = 589_760;
        let lpn_n = 10_805_248;
        let lpn_noise_weight = 1_319;
        let ggm_block_size = lpn_n / lpn_noise_weight;
        let setup_lpn_k = 25_000;
        let setup_lpn_n = 642_048;
        let setup_lpn_noise_weight = 2_508;
        let setup_ggm_block_size = setup_lpn_n / setup_lpn_noise_weight;
        let usable = lpn_n - lpn_k - lpn_noise_weight - 2;
        assert!(output_sub_equiv <= usable, "WYKW regular-LPN extend capacity exceeded");
        PhaseAParams {
            profile: PROFILE.into(),
            security_bits: 128,
            setup_lpn_estimated_security_bits: 140.646_864_307_606_42,
            setup_lpn_security_margin_bits: 12.646_864_307_606_42,
            extend_lpn_estimated_security_bits: 149.477_333_953_739_8,
            extend_lpn_security_margin_bits: 21.477_333_953_739_8,
            output_sub_equiv,
            lpn_k,
            lpn_n,
            lpn_noise_weight,
            // Two correlations are reserved by the F_p^2 consistency check.
            base_vole_len: lpn_k + lpn_noise_weight + 2,
            code_fanout: 10,
            ggm_block_size,
            ggm_depth: ceil_log2(ggm_block_size),
            setup_lpn_k,
            setup_lpn_n,
            setup_lpn_noise_weight,
            setup_ggm_block_size,
            setup_ggm_depth: ceil_log2(setup_ggm_block_size),
            consistency_reps: 2,
            noise_structure:
                "D_n,t regular: t disjoint blocks; one uniform nonzero F_p error at a uniform position per block"
                    .into(),
            full_field_combiner: "x=x0+phi*x1, m=m0+phi*m1, k=k0+phi*k1".into(),
            parameter_source:
                "regular LPN over log2(q)=64; main (k,n,t)=(589760,10805248,1319) from WYKW/Wolverine ePrint 2020/925 Section 5.2 Definition 1 and Section 6.1 Table 2; setup hardened to (25000,642048,2508) after Briaud-Oeygarden ePrint 2023/176 Section 5 found 126.44 bits for published k0=19870; Code Estimators commit 969ef60c30cb84c25502d6b7c968f43a362bb438 reports 140.646864 setup and 149.477334 main, margins 12.646864 and 21.477334 bits over 128"
                    .into(),
        }
    }
    ...
    pub fn tiny_for_test(output_sub_equiv: usize) -> PhaseAParams {
        ...
        let ggm_block_size = 32;
        let lpn_n = lpn_noise_weight * ggm_block_size;
        assert!(output_sub_equiv <= lpn_n - lpn_k - lpn_noise_weight - 2);
        let setup_lpn_k = 16;
        let setup_lpn_noise_weight = 4;
        let setup_ggm_block_size = 32;
        let setup_lpn_n = setup_lpn_noise_weight * setup_ggm_block_size;
        PhaseAParams {
            profile: format!("{PROFILE}-test"),
            security_bits: 128,
            setup_lpn_estimated_security_bits: 0.0,
            setup_lpn_security_margin_bits: 0.0,
            extend_lpn_estimated_security_bits: 0.0,
            extend_lpn_security_margin_bits: 0.0,
            output_sub_equiv,
            lpn_k,
            lpn_n,
            lpn_noise_weight,
            ...
            setup_ggm_depth: ceil_log2(setup_ggm_block_size),
            consistency_reps: 2,
            noise_structure: "test regular noise: one uniform nonzero per disjoint block".into(),
            full_field_combiner: "x=x0+phi*x1, m=m0+phi*m1, k=k0+phi*k1".into(),
...
mod tests {
    ...
    fn phase_b_outputs_satisfy_mac_and_counts_setup_bytes() {
        ...
        let out = expand_phase_b([0xB7u8; 32], [0x4Du8; 32], 48, 5, params).unwrap();
        let delta = out.verifier_delta;
