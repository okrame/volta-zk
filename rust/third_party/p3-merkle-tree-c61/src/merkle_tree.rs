    ...
    MerkleTree<F, W, M, N, DIGEST_ELEMS>
{
    ...
    /// Read-only access to the committed matrices for a prover-data
    /// lifecycle adapter.
    ///
    /// VOLTA C6SPX1 uses this only immediately after the pinned constructor
    /// returns, to write the matrices into a session-bound spill file before
    /// releasing the resident tree.  It does not alter tree construction.
    #[must_use]
    pub fn c61_spill_leaves(&self) -> &[M] {
        &self.leaves
    }

    /// Read-only access to the exact pinned digest layers for C6SPX1 spill.
    #[must_use]
    pub fn c61_spill_digest_layers(&self) -> &[Vec<[W; DIGEST_ELEMS]>] {
        &self.digest_layers
    }

    /// Read-only access to the exact pinned arity schedule for C6SPX1 spill.
    #[must_use]
    pub fn c61_spill_arity_schedule(&self) -> &[usize] {
        &self.arity_schedule
    }
}
