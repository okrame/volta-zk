...
`23,891,144` linear nodes.  Canonical normalization, independent verifier
reconstruction, cache constraint census, wrapper proof, local end-to-end and
...
| C6 Δ-residual inline certificate | **LEAN AMPLIFICATION + PRODUCT-CLOSURE / RUST RESIDUAL-IR / HIDDEN-U / ROOFLINE / PAIRED CODEC+STATE / SOURCE CENSUS / PAIRED COMPLETE SOURCE WITNESS GREEN; PROVER OPERATION TRACE EXACT; NORMALIZATION/VERIFIER/CACHE/WRAPPER PENDING; HARD STOP BEFORE POD** (2026-07-29) | Hide direct `auth_corrections` and Ligero `u_vectors`; one inline native-field wrapper; fixed-capacity committed KV cache; 17 accepts plus retry reserve; no deferred settlement | Retained Q=121 response **29,176,632 B**; new-payload budget **5,823,368 B**; implemented `pi_final` roof **4,409,824 B** under the owner cap **4,500,000 B**; complete response roof **33,586,456 B <=35,000,000 B**.  Q=121 gives **79.472744138609166392... bits**, preserving the **78.80929487391641-bit** floor after four amplified wrapper events.  Two independent tapes each fit **17 accepts + 4 aborts**, and paired PCG setup is **76,742,930 B**.  Lean is **3,257 jobs / 274 total / 38 C6**, zero `sorry`/`admit`; schema-v2 setup/state/certificate codecs bind paired ranges, atomic abort/retry, anti-fork state and idempotent ACK recovery.  Fixture first exchange is **76,743,367 B**.  Hidden-`u` removes exactly **17,235,968 B**, binds **12,686** NTT/`ip` relations and emits a **1,912-B** prequery plus one **32-B** seed.  The exact source census is **4,975,525 leaves / 673 ProductClosures / 22,339 triples / 8,170 zero closures**.  The clean paired complete-source reference extracts all **4,975,525 `(r,d,m)` leaves per coordinate** with no second model run and unchanged allocation schedule; the 673 ProductMask plaintexts are independently refreshed.  The full prover-side trace reaches exactly those censuses with **23,891,144 raw linear nodes** and no untracked closure operand.  This is still compiler input, not a succinct verifier: canonical plan normalization/digest, independent verifier equality, cache/wrapper proof, final bytes, real-PCG session, prover wall and hardware verdict remain pending. |
| C6 production T1 prover operation trace | **DIRTY LOCAL DIAGNOSTIC EXACT; PROVER OPERAND LINKAGE GREEN; NO RUN-OF-RECORD / NO DAG MILESTONE VERDICT** (2026-07-29) | Separate `c6-trace` build; canonical interleaved source ordinals; typed public/add/sub/scale propagation; central-only ProductClosure and ZeroBatch capture; unchanged ordinary build | Frozen local `100+50` reaches **4,975,525 sources / 23,891,144 raw linear nodes / 673 ProductClosures / 22,339 triples / 8,170 pre-mask zero roots**, with golden, transcript, allocation and paired-source digests unchanged.  Initial hard stops exposed and fixed only two trace-shape seams: `768` real sources versus a padded `1,024`-coefficient MLE vector, and an authenticated cache-fold boundary that previously discarded provenance.  No identity was inferred and no source was synthesized.  Default layouts remain exactly **32/24/16/24/32 B**; the full ordinary workspace and feature-specific MAC tests pass.  The diagnostic JSON was printed only, not persisted.  Raw node count/order is noncanonical and receives no timing credit.  Normalization, verifier reconstruction and a clean append-only plan record remain mandatory before this milestone can close. |
