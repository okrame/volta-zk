...
| P7 report + GPU budget model | **A100 roofline + fused MAC passed; LogUp/PCS open** (2026-07-11) | local report/budget model ✓; real-PCG cost model ✓; clean A100 P1/P6 baselines ✓; Goldilocks/F_p² roofline ✓; fused GEMM-MAC correctness + rho<=1.30 ✓; LogUp/PCS kernels and proving-path integration open | Packed download remains 144.8 MB. Required relative acceleration is **5.48× prefill / 3.97× decode**. A100 roofline: stream 55.48× CPU / chain 300.94×. Fused GEMM-MAC: weighted `rho_kernel=1.003`, shape range 0.983--1.009, exact output/correction reconstruction. Sources: `p7-gpu-roofline-2026-07-11-a43d105.json`, `p7-gpu-fused-epilogue-2026-07-11-bde5d7d.json`. Next: representative LogUp fraction-tree kernel, then PCS row/global + blake3; final go/no-go remains open. |
