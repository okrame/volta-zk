...
pub type MaskOracle<EF, M> =
    (<M as Mmcs<EF>>::Commitment, <M as Mmcs<EF>>::ProverData<RowMajorMatrix<EF>>);
...
/// Public affine representation of a hidden scalar claim.  C6.1 fixes the
/// authenticated target before verifier entropy, then carries only these two
/// coordinates through the dropped-linear-coefficient sumcheck transcript.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct AffineClaim<EF> {
    pub coefficient: EF,
    pub constant: EF,
}

impl<EF: Field> AffineClaim<EF> {
    pub const fn identity() -> Self {
        Self { coefficient: EF::ONE, constant: EF::ZERO }
    }

    pub fn evaluate(self, target: EF) -> EF {
        self.coefficient * target + self.constant
    }

    pub fn prelude(self, eps: EF, mu_tilde: EF) -> Self {
        Self { coefficient: eps * self.coefficient, constant: eps * self.constant + mu_tilde }
    }

    /// Replay `[c_0,c_2,...,c_d]` without materializing the target-dependent
    /// `c_1 = target - 2*c_0 - sum_{i>=2} c_i`.
    pub fn replay_round(self, wire: &[EF], gamma: EF) -> Self {
        let c0 = wire[0];
        let tail_at_one = wire[1..].iter().copied().sum::<EF>();
        let mut power = gamma * gamma;
        let mut tail_at_gamma = EF::ZERO;
        for &coefficient in &wire[1..] {
            tail_at_gamma += coefficient * power;
            power *= gamma;
        }
        Self {
            coefficient: gamma * self.coefficient,
            constant: c0 + gamma * (self.constant - c0.double() - tail_at_one) + tail_at_gamma,
        }
    }

    pub fn add_public(self, value: EF) -> Self {
        Self { coefficient: self.coefficient, constant: self.constant + value }
    }
}

/// Claimless verifier handoff.  No hidden target plaintext is accepted by
/// this API or absorbed into the challenger.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ZkAffineVerifierHandoff<EF> {
    pub randomness: Point<EF>,
    pub claimed_residual: AffineClaim<EF>,
    pub eps: EF,
}
    ...
    EF: Field,
{
    ...
    masks.iter().zip(gammas).map(|(mask, &gamma)| mask.iter().copied().horner(gamma)).sum()
}
...
where
    EF: Field,
{
    assert!(masks.iter().all(|mask| mask.len() == masks.first().map_or(0, Vec::len)));
...
pub fn mask_residual_covectors_from_shape<EF: Field>(
    ...
    gammas.iter().map(|gamma| gamma.powers().collect_n(mask_len)).collect()
}
...
mod tests {
        ...
        EF: Field,
    {
        ...
        let mut sum_future_endpoints: EF =
            masks.iter().map(|mask| mask[0].double() + mask[1..].iter().copied().sum::<EF>()).sum();
        ...
        for (round_idx, (s_j, &gamma_j)) in masks.iter().zip(gammas).enumerate() {
            ...
            target = h.iter().rev().copied().fold(EF::ZERO, |acc, coeff| acc * gamma_j + coeff);

            let s_j_at_gamma =
                s_j.iter().rev().copied().fold(EF::ZERO, |acc, coeff| acc * gamma_j + coeff);
    ...
    fn mask_residual_closed_form_matches_round_recurrence() {
        let masks = vec![
            vec![EF::from_u64(3), EF::from_u64(5), EF::from_u64(7), EF::from_u64(11)],
            vec![EF::from_u64(13), EF::from_u64(17), EF::from_u64(19), EF::from_u64(23)],
            vec![EF::from_u64(29), EF::from_u64(31), EF::from_u64(37), EF::from_u64(41)],
        ];
