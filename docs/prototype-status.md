...
projection is `63,994,751 B`, but receives zero setup credit until an encoder,
strict decoder and materialized full artifact reproduce it.  Cache constraint
...
| C6 Δ-residual inline certificate | **LEAN AMPLIFICATION + PRODUCT-CLOSURE / RUST RESIDUAL-IR / HIDDEN-U / ROOFLINE / PAIRED CODEC+STATE / SOURCE CENSUS / PAIRED COMPLETE SOURCE WITNESS GREEN; PARAMETERIZED V2 TWO-SEED IDENTITY GREEN; CANONICAL PLAN CODEC/CACHE/WRAPPER PENDING; HARD STOP BEFORE POD** (2026-07-29) | Hide direct `auth_corrections` and Ligero `u_vectors`; one inline native-field wrapper; fixed-capacity committed KV cache; 17 accepts plus retry reserve; no deferred settlement | Retained Q=121 response **29,176,632 B**; new-payload budget **5,823,368 B**; implemented `pi_final` roof **4,409,824 B** under the owner cap **4,500,000 B**; complete response roof **33,586,456 B <=35,000,000 B**.  Q=121 gives **79.472744138609166392... bits**, preserving the **78.80929487391641-bit** floor after four amplified wrapper events.  Two independent tapes each fit **17 accepts + 4 aborts**, and paired PCG setup is **76,742,930 B**.  Lean is **3,257 jobs / 274 total / 38 C6**, zero `sorry`/`admit`; schema-v2 setup/state/certificate codecs bind paired ranges, atomic abort/retry, anti-fork state and idempotent ACK recovery.  Fixture first exchange is **76,743,367 B**.  Hidden-`u` removes exactly **17,235,968 B**, binds **12,686** NTT/`ip` relations and emits a **1,912-B** prequery plus one **32-B** seed.  The exact source census remains **4,975,525 leaves / 673 ProductClosures / 22,339 triples / 8,170 zero closures**.  Historical v1 exact-instance equality remains clean at `0b4bb678...2f489bf`.  Full v2 seed-24/25 runs have identical topology `bcdd169f...c57af344` and distinct exact/instance digests, with **28,845,631 nodes / 23,874,780 reachable operations per side / 1,436 public slots / 10,828,852 scalar slots**.  A straightforward bit-packed plan is **88,934,137 B** and fails the client-parameter allowance; signed source deltas plus unit-operand flags project **63,994,751 B**, leaving **9,262,319 B**, but explicitly have `setup_fit_credit:false` until canonical encode/decode and a materialized artifact reproduce the count/digest.  Cache/wrapper proof, final bytes, real-PCG session, prover wall and hardware verdict remain pending. |
