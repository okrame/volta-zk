...
remain typed pending values rather than accepted PCS outputs.  The next
executable seam is now frozen as one witness-independent atomic event replay
with provider first-message/folded-state sinks and a vector-free client
terminal sink.  Its implementation and differential are next, followed by
synchronized packed-PCS authenticated-output assembly.
