...
pooled allocation.  The historical attention path still constructs
`CacheSegK`.  The first inline injection audit has also preregistered a
zero-byte ordering repair: `C6FT1` starts from public kinds/count, accumulates
the challenge-dependent runtime identity with the target slots, and seals it
after padding but before `rho`.  That split typestate and its early-finish /
wrong-runtime-identity negatives are now locally green at the unchanged
**18,480 B**.  Its C6-specific key-vector removal, the
resident provider path, production transition, envelope/backend, local
end-to-end and hardware gates remain pending.
