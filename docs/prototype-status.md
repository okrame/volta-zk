...
audit is **345 total / 106 C6** named targets.  Strict `C6PS1` transcript
migration, typed pending-target replacement, `CacheSegK` removal, the resident
...
| C6 Δ-residual inline certificate | **SCALED SOURCE-BOUND C6PS1 → C6PC2 → 72-SLOT C6LNK2 → PAIRED PCS GREEN; FACTORIZED FOLD COMPILER GREEN; C6PS1 MIGRATION / CACHESEGK REMOVAL PENDING; NO POD** (2026-07-31) | Hide direct `auth_corrections` and Ligero `u_vectors`; one inline native-field wrapper; fixed-capacity committed KV cache; 17 accepts plus retry reserve; no deferred settlement | Q=121 keeps complete soundness at **79.472744138609180097... bits**.  The descendant has **pi_final 4,479,466 B** and complete response **33,656,098 B <=35,000,000 B**; known first exchange remains **146,058,504 B**.  `C6LNK2` is exact at **72 relations / six groups / 1,939,733 B per chain / 3,879,466 B paired PCS / 3,570 B link overhead**.  The amended cache event is **653 roots / 426,409 two-repetition numerator <2^19**.  Strict production core `C6PC2` is **3,506 B / 104 full correlations per tape**; the next strict `C6PS1` remains **304 B / zero correlations**, for a **3,810-B** source-bound package.  The 6-round source-bound reference remains **1,506 B / 32 per tape / 64 pending claims** and replaces every former cache/cache-auxiliary stand-in through the real packed link/PCS fixture.  Full Lean is **3,261 jobs / 345 audited targets (106 C6)**; exact C6 budget tests are **9/9**.  Ordinary/trace PCS are **177/0/1** and **178/0/1**; ordinary/trace proto are **143/0/1** and **151/0/1**; workspace and ordinary/feature all-target checks are green.  The conservative pre-backend floor remains **11.1793342101 s**.  Strict transcript migration, resident provider, `CacheSegK` and response-field removal, measured timing and hardware gates remain pending. |
