...
/// Public side of the final target identity.  C6.1 checks this equation only
/// after lifting the affine carried target onto the designated MAC key.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct BaseCaseClaimlessClosure<EF> {
    pub combined: EF,
    pub shifted_masked_claim: EF,
    pub gamma: EF,
}
    ...
    MT: Mmcs<F>,
{
    ...
    pub fn verify_claimless<Challenger>(
        &self,
    ...
    ) -> Result<BaseCaseClaimlessClosure<EF>, BaseCaseZkError>
    where
            ...
            + CanObserve<MT::Commitment>,
    {
        ...
        let num_masks: usize = self.config.mask_groups.iter().map(|group| group.width).sum();
        ...
        let blinded = |kind, actual: usize, expected: usize| {
            if actual == expected {
                ...
                Err(BaseCaseZkError::BlindedLengthMismatch { kind, expected, actual })
            }
        };
        blinded("message", proof.blinded_message.len(), code.message_len)?;
        blinded("randomness", proof.blinded_randomness.len(), code.randomness_len)?;
        // One shape per carried mask, in group order; the counts pinned above
        // guarantee both zips cover every covector and reveal.
        let shapes =
            self.config.mask_groups.iter().flat_map(|group| repeat_n(&group.shape, group.width));
        for ((covector, mask), shape) in mask_covectors.iter().zip(&proof.blinded_masks).zip(shapes)
        {
            ...
            blinded("mask randomness", mask.randomness.len(), shape.randomness_len)?;
        }
        ...
        let claimless_closure =
            BaseCaseClaimlessClosure { combined, shifted_masked_claim: proof.masked_claim, gamma };
        ...
        let openings = |kind, actual: usize, expected: usize| {
            if actual == expected {
                ...
                Err(BaseCaseZkError::OpeningCountMismatch { kind, expected, actual })
            }
        };
        ...
        let fresh_main_dims = vec![Dimensions { height: code.domain_size, width: 1 }];
        ...
        for (group_index, (group, pair)) in
            self.config.mask_groups.iter().zip(&proof.mask_openings).enumerate()
        {
            let positions = get_challenge_stir_queries::<Challenger, F>(
            ...
            let dims = vec![Dimensions { height: group.shape.domain_size, width: group.width }];
