#!/usr/bin/env bash
# Non-session A100 all-lane calibration for C62GW4. Provider-only caches and
...
if [[ -n $(git status --porcelain=v1 --untracked-files=all) ]]; then
  echo "C62GW4 calibration requires a clean source tree" >&2
  exit 2
fi
if [[ $RECORD == "$REPO_ROOT"/* || -e $RECORD ]]; then
  echo "C62GW4 calibration record must be create-new and outside the repository" >&2
  exit 2
fi
if [[ ! -x /usr/local/cuda/bin/nvcc ]]; then
  echo "C62GW4 calibration requires /usr/local/cuda/bin/nvcc" >&2
  exit 2
fi
GPU=$(nvidia-smi --query-gpu=name,memory.total --format=csv,noheader,nounits)
if [[ $GPU != *"A100-SXM4-80GB"* ]]; then
  echo "C62GW4 calibration requires one A100-SXM4-80GB; found: $GPU" >&2
  exit 2
fi
...
timeout 1800 cargo test --release --manifest-path rust/Cargo.toml \
  -p volta-pcs --lib \
  --features cuda,c61-p3-authenticated-reference,c6-trace \
  c61_authenticated_whir_p3::tests::c62gw4_a100_all_lane_calibration \
  -- --ignored --exact --nocapture --test-threads=1

test -s "$RECORD"
echo "C62GW4_CALIBRATION_RECORD: $RECORD"
