...
use super::verifier::BaseCaseClaimlessClosure;
    ...
    StandardUniform: Distribution<EF>,
{
    ...
    pub fn prove<Dft, Challenger, R>(
        ...
        masked_claim_shift: EF,
        open_source: impl FnOnce(&[usize]) -> QueryOpenings<F, EF, MT::MultiProof>,
        challenger: &mut Challenger,
        rng: &mut R,
    ) -> (BaseCaseZkProof<F, EF, MT>, BaseCaseClaimlessClosure<EF>)
    where
        ...
        R: Rng,
    {
        ...
        let mut masked_claim =
            dot_product::<EF, _, _>(fresh_message.iter().copied(), source_covector.iter().copied());
        ...
        masked_claim += masked_claim_shift;
        ...
        let blind = |fresh: &[EF], hidden: &[EF]| -> Vec<EF> {
            fresh.iter().zip(hidden).map(|(&fresh, &hidden)| fresh + gamma * hidden).collect()
        };
        ...
        let mut combined = dot_product::<EF, _, _>(
            blinded_message.iter().copied(),
            source_covector.iter().copied(),
        );
        for (blinded, witness) in
            blinded_masks.iter().zip(masks.iter().flat_map(|witness| witness.covectors.iter()))
        {
            combined +=
                dot_product::<EF, _, _>(blinded.message.iter().copied(), witness.iter().copied());
        }
        let claimless_closure =
            BaseCaseClaimlessClosure { combined, shifted_masked_claim: masked_claim, gamma };

        // PoW before the spot checks.
        //
        //     pow_bits = 0  ->  no grind, zero witness on the wire
        let pow_witness =
            if self.config.pow_bits > 0 { challenger.grind(self.config.pow_bits) } else { F::ZERO };
