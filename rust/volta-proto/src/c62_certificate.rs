//! Canonical C6.2 designated-verifier certificate.
...
use crate::{
    C61RetainedResponseBinding, C62ResponseProofEnvelope, C6CacheHead, C6CorrelationRange,
    C6PairedCorrelationRanges, C6PairedDeltaResidual, C6RetainedResponseProof, C6Workload,
    C6_MAX_CONTEXT,
};

pub const C62_RETAINED_NON_PCS_RESPONSE_BYTES: u64 = crate::C62_RETAINED_RESPONSE_BYTES as u64;
pub const C62_PUBLIC_ARGUMENT_ABSOLUTE_MAX_BYTES: u64 = 15_157_896;
pub const C62_CERTIFICATE_STRICT_MAX_BYTES: u64 = 21_999_999;
pub const C62_NATIVE_CERTIFICATE_VERSION: u16 = 1;
pub const C62_NATIVE_WRAPPER_QUERIES: u16 = 86;
pub const C62_NATIVE_CERTIFICATE_FRAMING_BYTES: u64 = 793;
pub const C62_NATIVE_STRICT_PI_FINAL_MAX_BYTES: u64 =
    C62_NATIVE_CERTIFICATE_FRAMING_BYTES + crate::C62_RESPONSE_PROOF_ENVELOPE_MAX_BYTES;

const C62_NATIVE_CERTIFICATE_MAGIC: &[u8] = b"VOLTA-C62-CERT-v1";
const C62_NATIVE_RETAINED_MAX_BYTES: u64 =
    C62_RETAINED_NON_PCS_RESPONSE_BYTES + C62_PUBLIC_ARGUMENT_ABSOLUTE_MAX_BYTES;

#[derive(Clone, Debug, PartialEq, Eq)]
pub struct C62CertificateError(String);

impl C62CertificateError {
    fn new(message: impl Into<String>) -> Self {
        Self(message.into())
    }
}

impl fmt::Display for C62CertificateError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter.write_str(&self.0)
    }
}

impl std::error::Error for C62CertificateError {}

type Result<T> = std::result::Result<T, C62CertificateError>;

#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct C62NativeWrapperCommitments {
...
impl C62NativeWrapperCommitments {
    ...
    pub(crate) fn validate(self) -> Result<()> {
        ...
        .contains(&[0; 32])
        {
            return Err(C62CertificateError::new("C62NFC1 wrapper contains a zero digest"));
        }
        Ok(())
    }
}
...
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct C62NativeFinalCertificate {
    pub version: u16,
    ...
    pub wrapper: C62NativeWrapperCommitments,
...
impl C62NativeFinalCertificate {
    pub fn seal(mut self) -> Result<Self> {
        self.retained_transcript_digest = c62_retained_digest(&self.retained_transcript);
        self.proof_envelope_digest = c62_proof_digest(&self.proof_envelope);
    ...
    pub fn decode(bytes: &[u8]) -> Result<Self> {
        if bytes.len() as u64 > C62_CERTIFICATE_STRICT_MAX_BYTES {
            return Err(C62CertificateError::new("C62NFC1 exceeds its certificate cap"));
        }
        let mut input = Decoder::new(bytes);
        if input.take(C62_NATIVE_CERTIFICATE_MAGIC.len())? != C62_NATIVE_CERTIFICATE_MAGIC {
            return Err(C62CertificateError::new("wrong C62NFC1 certificate magic"));
        }
        let certificate = Self {
            ...
            retained_transcript: input.blob(C62_NATIVE_RETAINED_MAX_BYTES as usize)?,
            proof_envelope: input.blob(crate::C62_RESPONSE_PROOF_ENVELOPE_MAX_BYTES as usize)?,
        };
        input.finish()?;
        certificate.validate()?;
        if certificate.encode_unchecked() != bytes {
            return Err(C62CertificateError::new("noncanonical C62NFC1 encoding"));
        }
        Ok(certificate)
    }
    ...
    pub fn digest(&self) -> Result<[u8; 32]> {
        Ok(hash_parts(b"volta-zk/c6.2/final-certificate/v1", &[&self.encode()?]))
    }
    ...
    pub fn retained_response(&self) -> &[u8] {
        &self.retained_transcript[..C62_RETAINED_NON_PCS_RESPONSE_BYTES as usize]
    }
    ...
    pub fn retained_response_binding(&self) -> C61RetainedResponseBinding {
        C61RetainedResponseBinding::from_c62_bytes(self.retained_response())
            .expect("validated C62NFC1 has a strict retained response")
    }
    ...
    pub fn decoded_proof_envelope(&self) -> C62ResponseProofEnvelope {
        C62ResponseProofEnvelope::decode(&self.proof_envelope)
            .expect("validated C62NFC1 has a strict proof envelope")
    }
    ...
    pub fn compute_transition_statement_digest(&self) -> [u8; 32] {
        hash_parts(b"volta-zk/c6.2/transition-statement/v1", &[&self.encode_statement()])
    }

    pub fn validate(&self) -> Result<()> {
        if self.version != C62_NATIVE_CERTIFICATE_VERSION
            || self.wrapper_queries != C62_NATIVE_WRAPPER_QUERIES
        {
            return Err(C62CertificateError::new("wrong C62NFC1 version or query profile"));
        }
        if [
        ...
        .contains(&[0; 32])
        {
            return Err(C62CertificateError::new("C62NFC1 contains a zero required digest"));
        }
        self.old_head.validate().map_err(|error| C62CertificateError::new(error.to_string()))?;
        self.new_head.validate().map_err(|error| C62CertificateError::new(error.to_string()))?;
        ...
        {
            return Err(C62CertificateError::new("C62NFC1 predecessor differs from genesis"));
        }
        self.correlation_ranges
            .validate()
            .map_err(|error| C62CertificateError::new(error.to_string()))?;
        self.workload.validate().map_err(|error| C62CertificateError::new(error.to_string()))?;
        ...
        if self.new_head.epoch
            != self
                ...
                .epoch
                .checked_add(1)
                .ok_or_else(|| C62CertificateError::new("C62NFC1 cache epoch overflows"))?
        ...
        {
            return Err(C62CertificateError::new("C62NFC1 does not advance its predecessor"));
        }
        if self.retained_transcript.len() <= C62_RETAINED_NON_PCS_RESPONSE_BYTES as usize
            || self.retained_transcript.len() as u64 > C62_NATIVE_RETAINED_MAX_BYTES
        {
            return Err(C62CertificateError::new("C62NFC1 retained partition violates its cap"));
        }
        C6RetainedResponseProof::decode_c62(self.retained_response())
            .map_err(|error| C62CertificateError::new(error.to_string()))?;
        C62ResponseProofEnvelope::decode(&self.proof_envelope)
            .map_err(|error| C62CertificateError::new(error.to_string()))?;
        if self.retained_transcript_digest != c62_retained_digest(&self.retained_transcript)
            || self.proof_envelope_digest != c62_proof_digest(&self.proof_envelope)
        {
            return Err(C62CertificateError::new("C62NFC1 payload digest mismatch"));
        }
        ...
        {
            return Err(C62CertificateError::new("C62NFC1 transition or head digest mismatch"));
        }
        let encoded_len = self.encode_unchecked().len() as u64;
        let proof_boundary = C62_NATIVE_CERTIFICATE_FRAMING_BYTES
            .checked_add(self.proof_envelope.len() as u64)
            .ok_or_else(|| C62CertificateError::new("C62NFC1 proof boundary overflows"))?;
        if encoded_len > C62_CERTIFICATE_STRICT_MAX_BYTES
            || proof_boundary > C62_NATIVE_STRICT_PI_FINAL_MAX_BYTES
        {
            return Err(C62CertificateError::new("C62NFC1 size cap exceeded"));
        }
        Ok(())
    }
    ...
    fn encode_statement(&self) -> Vec<u8> {
        ...
        out.raw(b"VOLTA-C62-STATEMENT-v1");
    ...
    fn encode_unchecked(&self) -> Vec<u8> {
        let mut out = Encoder::new();
        out.raw(C62_NATIVE_CERTIFICATE_MAGIC);
...
fn c62_retained_digest(bytes: &[u8]) -> [u8; 32] {
    hash_parts(b"volta-zk/c6.2/retained-transcript/v1", &[bytes])
}

fn c62_proof_digest(bytes: &[u8]) -> [u8; 32] {
...
impl<'a> Decoder<'a> {
    ...
    pub(crate) fn take(&mut self, len: usize) -> Result<&'a [u8]> {
        let end = self
            .offset
            .checked_add(len)
            .ok_or_else(|| C62CertificateError::new("C62NFC1 decoder offset overflows"))?;
        if end > self.bytes.len() {
            return Err(C62CertificateError::new("truncated C62NFC1 certificate"));
        }
    ...
    pub(crate) fn fp2(&mut self) -> Result<Fp2> {
        ...
        let c1 = self.u64()?;
        if c0 >= P || c1 >= P {
            return Err(C62CertificateError::new("noncanonical C62NFC1 field element"));
        }
    ...
    pub(crate) fn blob(&mut self, max_len: usize) -> Result<Vec<u8>> {
        ...
        if len > max_len {
            return Err(C62CertificateError::new("C62NFC1 blob exceeds its cap"));
        }
...
mod tests {
    ...
    fn certificate() -> C62NativeFinalCertificate {
        ...
        let proof_envelope = C62ResponseProofEnvelope::new(
            vec![0x51],
            vec![0x50; crate::C62_RESPONSE_PRODUCT_COORDINATE_ONE_BYTES as usize],
            vec![0x52; crate::C62_RESPONSE_RESIDUAL_PENDING_BYTES as usize],
            vec![0x53; crate::C62_RESPONSE_CACHE_SOURCE_BYTES as usize],
            vec![0x54],
            vec![0x55; crate::C62_RESPONSE_CACHE_FOLD_TARGET_BYTES as usize],
            vec![0x56; crate::C62_RESPONSE_AUTHENTICATED_LINK_BYTES as usize],
        )
        .unwrap()
        .encode()
        .unwrap();
    ...
    #[test]
    fn c62_nfc1_round_trip_is_wire_neutral_and_cross_version_strict() {
        let certificate = certificate();
        let bytes = certificate.encode().unwrap();
        assert_eq!(C62NativeFinalCertificate::decode(&bytes).unwrap(), certificate);
        assert_eq!(
            bytes.len() as u64
                - certificate.retained_transcript.len() as u64
                - certificate.proof_envelope.len() as u64,
            C62_NATIVE_CERTIFICATE_FRAMING_BYTES,
        );
        assert!(crate::C61NativeFinalCertificate::decode(&bytes).is_err());
        assert!(crate::C6FinalCertificate::decode(&bytes).is_err());

        let mut changed = bytes;
        changed[0] ^= 1;
        assert!(C62NativeFinalCertificate::decode(&changed).is_err());
    }
}
