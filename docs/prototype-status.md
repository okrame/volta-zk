...
One complete same-pass T1 diagnostic proves that the lightweight recorder
reproduces both exact raw censuses and the canonical instance digest without
changing authenticated-value layouts.  Bound ordinary-build provider
overhead, cache constraint census, wrapper proof, local end-to-end and
...
| C6 role-local instance extraction codec | **CLEAN MAP PAIR + CLEAN RUNTIME SEED-24 PASS; RUNTIME SEED-25 / BOUND ORDINARY TIMING PENDING** (2026-07-29) | No response-value vector on wire; role-specific model-global maps; maximal-`+1` run codec fixed before measurement; strict ordinary-build decoder and thread-local value recorder | Clean map seed-24/25 runs reconstruct distinct instance digests and emit stable role artifacts.  The verifier map is **5,320,386 B** with **1,466 / 10,828,876 raw public/scalar slots**, **1,436 / 10,828,852 canonical slots** and **262 / 2,552,791 runs**; BLAKE3 `17ed0942...7183535`.  Client topology + map are **69,315,137 B**; paired PCG + setup envelope total **146,058,504 B**, leaving **3,941,496 B**.  Clean schema-10 runtime seed-24 record `c6-runtime-instance-recorder-seed24-2026-07-29-3b01789.json`, SHA-256 `f4ea526eabdb7fa3a998d5d6c97a8d84bd3f3e6ba72e5a427da186d79110681b`, captures exactly **1,466 / 10,837,046** prover and **1,466 / 10,828,876** verifier events and reconstructs `7a21189b...45cacaa`; seed 25 remains pending.  Full trace was enabled, so this gives seam correctness but no runtime/prover-time, residual or production credit. |
...
- **2026-07-29 — C6 lightweight runtime instance seam is exact on full T1;
  ...
  **1,466 public + 10,828,876 scalar** verifier events.  Applying the clean
  ...
  Clean schema-10 seed-24 record
  ...
  reproduces the result with `git_dirty:false`, `diagnostic:false`,
  `pod_contacted:false` and `all_pass:true`; clean seed 25 remains pending.
  Because `c6-trace` is enabled and `timing_credit:false`, this closes runtime
  seam equality only.  A bound ordinary-build run with graph tracing
  disabled, provider overhead, compiled residual and production timing
  remain pending.  No provider/pod was contacted.
