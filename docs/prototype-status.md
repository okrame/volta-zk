...
green; the strict codec/source-stream gates, `CacheSegK` removal, the resident
...
| C6 Δ-residual inline certificate | **SCALED SOURCE-BOUND C6PS1 V2 → C6PC2 → 72-SLOT C6LNK2 → PAIRED PCS GREEN; FACTORIZED FOLD COMPILER + STRICT POST-RHO TRANSCRIPT + ROLE-TYPED TARGET/STEP-WISE 24-ROUND ORCHESTRATION GREEN; C6FT1 FORMAL GREEN, STRICT CODEC PENDING; CACHESEGK REMOVAL PENDING; NO POD** (2026-07-31) | Hide direct `auth_corrections` and Ligero `u_vectors`; one inline native-field wrapper; fixed-capacity committed KV cache; 17 accepts plus retry reserve; no deferred settlement | Q=121 keeps complete soundness at **79.472744138609180097... bits**.  The descendant has **pi_final 4,479,466 B** and complete response **33,656,098 B <=35,000,000 B**; known first exchange remains **146,058,504 B**.  `C6LNK2` is exact at **72 relations / six groups / 1,939,733 B per chain / 3,879,466 B paired PCS / 3,570 B link overhead**.  The amended cache event is **653 roots / 426,409 two-repetition numerator <2^19**.  Strict production core `C6PC2` is **3,506 B / 104 full correlations per tape**; strict `C6PS1` v2 remains **304 B / zero correlations**, for a **3,810-B** source-bound package.  Per repetition its fold K/V corrections follow its scalar root and precede its relation point; append K/V follows that point; predecessor owner is canonical zero.  The runtime seam now carries role-typed authenticated targets, pairs both tape schedules and imports them through a source digest bound to the trace identity.  A scaled cache round state is driven only by the response-global 24-round coordinator.  The preregistered `C6FT1` frame adds a fixed **18,480 B** inside the existing 600,000-B non-PCS allocation and **zero fresh correlations**; it makes each individual target key available before ΠProd and deterministically feeds the later C6PS1 fold.  The individual-key and two-stage correction identities are now audited.  Registered wrapper use remains **622 /39,116 full correlations per tape**.  The 6-round source-bound reference remains **1,506 B / 32 per tape / 64 pending claims** and replaces every former cache/cache-auxiliary stand-in through the real packed link/PCS fixture.  Full Lean is **3,261 jobs / 347 audited targets (108 C6)**, audit SHA-256 `df0e4a4e7278c1f7a4f5be6ffc57d392c2aec332df80fe761fe8688ef07ada29`; exact C6 budget tests are **9/9**.  Ordinary/trace PCS are **179/0/1** and **180/0/1**; ordinary/trace proto remain **143/0/1** and **152/0/1**.  The conservative pre-backend floor remains **11.1793342101 s**.  `C6FT1` strict codec/source streaming, the resident provider, `CacheSegK` and response-field removal, production 24-round execution, measured timing and hardware gates remain pending. |
