...
use super::data::{AffineClaim, ZkAffineVerifierHandoff, ZkSumcheckData, ZkVerifierHandoff};
    ...
    EF: ExtensionField<F>,
{
    ...
    pub fn new_prefix(table_shapes: &[TableShape]) -> Self {
        Self {
            inner: Verifier::new(table_shapes, LayoutStrategy::new(true, VariableOrder::Prefix)),
        }
    }
    ...
    pub fn new_suffix(table_shapes: &[TableShape]) -> Self {
        Self {
            inner: Verifier::new(table_shapes, LayoutStrategy::new(false, VariableOrder::Suffix)),
        }
    }
    ...
    fn validate_shape(
        ...
        if zk_data.ell_zk != ell_zk {
            return Err(SumcheckError::EllZkMismatch { expected: ell_zk, actual: zk_data.ell_zk });
        }
        ...
        Ch: FieldChallenger<F> + GrindingChallenger<Witness = F> + CanObserve<M::Commitment>,
    {
        ...
        Ok(ZkVerifierHandoff { randomness: Point::new(randomness), claimed_residual: target, eps })
    }

    fn replay_affine_claim_unchecked<M, Ch>(
        zk_data: &ZkSumcheckData<F, EF>,
        mask_commitment: &M::Commitment,
        folding_factor: usize,
        pow_bits: usize,
        claimed_sum: AffineClaim<EF>,
        challenger: &mut Ch,
    ) -> Result<ZkAffineVerifierHandoff<EF>, SumcheckError>
    where
        M: Mmcs<EF>,
        Ch: FieldChallenger<F> + GrindingChallenger<Witness = F> + CanObserve<M::Commitment>,
    {
        ...
        let mut target = claimed_sum.prelude(eps, zk_data.mu_tilde);
        let mut randomness: Vec<EF> = Vec::with_capacity(folding_factor);
        ...
        for (j_idx, wire) in zk_data.round_coefficients.iter().enumerate() {
            ...
            let gamma_j: EF = challenger.sample_algebra_element();
            target = target.replay_round(wire, gamma_j);
            randomness.push(gamma_j);
        }

        Ok(ZkAffineVerifierHandoff {
    ...
    /// Replay a C6.1 claimless batch.  Unlike [`Self::verify_claim`], this API
    /// cannot accept or absorb a target plaintext; its output remains affine
    /// in the fixed authenticated opening target.
    #[allow(clippy::too_many_arguments)]
    pub fn verify_affine_claim<M, Ch>(
        zk_data: &ZkSumcheckData<F, EF>,
        mask_commitment: &M::Commitment,
        ell_zk: usize,
        folding_factor: usize,
    ...
    {
        Self::validate_shape(zk_data, ell_zk, folding_factor, pow_bits)?;
        Self::replay_affine_claim_unchecked::<M, _>(
            zk_data,
            mask_commitment,
            folding_factor,
            pow_bits,
...
mod tests {
    ...
    fn verifier_strategy_matches_non_private_layouts() {
        ...
        assert_ne!(zk_prefix.strategy().variable_order, zk_suffix.strategy().variable_order,);
    ...
    fn forged_pow_witness_rejected_case(binding: VariableOrder) {
        ...
        let mut run =
            run_prover(binding, n_vars, folding_factor, ell_zk, 0, num_virtual, pow_bits, seed);
    ...
    fn rbr_tampering_changes_verifier_output_case(
        ...
        let run = run_prover(binding, n_vars, folding_factor, ell_zk, 0, num_eqs, pow_bits, seed);
