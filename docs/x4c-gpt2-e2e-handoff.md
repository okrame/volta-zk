...
admitted all five rebuilt roots and the fresh online CUDA context, then
stopped fail-closed in warm-up because the response traffic gate incorrectly
required zero D2D copies and zero device-generated bytes. It emitted no
online JSON and cannot resume the burned connection or released in-memory
state.
...
**35,727,436,640 B** device-generated output—from the canonical gather plan
and response geometry. The dedicated accelerated validator requires the
top-level expectations, execution receipt and native counters to agree
exactly. This is accounting-only, not a protocol or cryptographic change.
...
sample outputs, rebuilt canonical-frontier digests and four derived hash
keys. These are device kernel writes, not file staging, host I/O, transcript
messages or new proof bytes.
