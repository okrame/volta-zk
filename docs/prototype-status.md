...
| C6 production T1 canonical prover plan | **DIRTY LOCAL DIAGNOSTIC EXACT; NORMALIZER + MUTATION TESTS GREEN; VERIFIER EQUALITY / RUN-OF-RECORD PENDING** (2026-07-29) | Separate `c6-trace` build; frozen ProductClosure-then-ZeroBatch terminal order; iterative post-order; no algebraic simplification/hash-consing; unchanged ordinary build | Frozen local `100+50` preserves **4,975,525 sources / 23,891,144 raw linear nodes / 673 ProductClosures / 22,339 triples / 8,170 pre-mask zero roots** and emits canonical digest `0b4bb67835d315807e81d2c3457b5c53bcd82036622307d6f91dec2e62f489bf`.  The compact identity has **28,845,583 canonical nodes**; **23,874,732** raw operations are reachable and the preregistered dead-code rule omits **16,412**.  Synthetic tests bind operand/root/source order, preserve graph sharing, ignore raw-ID/dead allocation changes, and reject future/out-of-range/untracked tokens plus mask misuse.  Golden, transcript, allocation and paired-source digests are unchanged.  JSON was printed only; no record or timing claim exists.  Independent verifier trace equality remains mandatory before this milestone closes. |
