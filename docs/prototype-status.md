...
| P7 report + GPU budget model | **pre-cloud local complete** (2026-07-07; real-PCG/cloud open) | local report/budget model ✓; PCS byte formula reproduces P6 ✓; static-query-cache marginal accounting in Rust ✓; Q=150 exploratory profile measured ✓; mock-PCG lower-bound measured ✓; decode marginal profiled ✓; no protocol/soundness parameter change without separate ledger entry | Baseline clean P6 `benchmarks/results/p6-2026-07-07-382bb56.json`: current packed download 144.8 MB; PCS formula matches 66.733 MB; measured quick Q=150 PCS 57.823 MB; decode marginal 22.253 MB = 445,067 B/token, dominated by auth corrections 20.902 MB; projections: Q=150 same-rate 135.9 MB, RLC 130.9 MB, Q+RLC 122.0 MB, static-query-cache marginal 110.8 MB, cache+RLC marginal 96.9 MB. GPU sensitivity from this baseline: targets need prover/native relative speedup 3.67× prefill, 2.60× decode. Mock-PCG lower-bound `p7-mock-pcg-2026-07-07-d16a69c.json`: 0.351 s for 8.48 M sub + 176.9 K full expanded both sides (not real-PCG). Recommendation: proceed to cloud spikes only; final go/no-go still needs real-PCG or explicit external budget plus GPU roofline/native-baseline measurements. |
...
- **2026-07-07 (P7 pre-cloud local complete)**:
  ...
  **22,253,392 B = 445,067 B/token**, with top non-PCS labels:
  ...
  report is regenerated after this clean checkpoint so it can carry
  `git_dirty:false`. Local P7 work before cloud is complete except for a
  real Ferret/silent-VOLE measurement, which is unavailable in this repo and
  remains an explicit cloud/external budget item.
