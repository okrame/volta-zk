...
| C6 role-local instance extraction codec | **CLEAN TWO-SEED MAP PASS + FULL T1 RUNTIME DIAGNOSTIC PASS; CLIENT SETUP FIT CLOSED; BOUND ORDINARY TIMING PENDING** (2026-07-29) | No response-value vector on wire; role-specific model-global maps; maximal-`+1` run codec fixed before measurement; strict ordinary-build decoder and thread-local value recorder | Clean seed-24/25 runs reconstruct distinct instance digests and emit stable role artifacts.  The verifier map is **5,320,386 B** with **1,466 / 10,828,876 raw public/scalar slots**, **1,436 / 10,828,852 canonical slots** and **262 / 2,552,791 runs**; BLAKE3 `17ed0942...7183535`.  Client topology + map are **69,315,137 B**; paired PCG + setup envelope total **146,058,504 B**, leaving **3,941,496 B**.  Records `c6-instance-extraction-seed24-2026-07-29-3d8e2ea.json` / `c6-instance-extraction-seed25-2026-07-29-1817975.json`, SHA-256 `d641f0bc...b71921c` / `a4938bdd...533280`, are clean PASSes.  A dirty stdout-only schema-10 seed-24 run captures exactly **1,466 / 10,837,046** prover and **1,466 / 10,828,876** verifier events and reconstructs `7a21189b...45cacaa` on both sides.  Full trace was enabled, so this gives seam correctness but no runtime/prover-time, residual or production credit. |
...
- **2026-07-29 — C6 lightweight runtime instance seam is exact on full T1;
  ...
  **1,466 public + 10,828,876 scalar** verifier events.  Applying the clean
  ...
  This was a dirty stdout-only schema-10 diagnostic at source `c8b2e27`,
  with `c6-trace` enabled and `timing_credit:false`.  It therefore closes
  runtime seam equality only.  A clean bound ordinary-build run with graph
  tracing disabled, provider overhead, compiled residual and production
  timing remain pending.  No provider/pod was contacted.
