...
single-arena amendment is now frozen before coordinator code; the original
512-MiB peak remains unchanged.  Fused `C6RSC3` integration is next, followed
by synchronized packed-PCS authenticated-output assembly.
...
| C6 round-synchronous fused arena | **OWNERSHIP AMENDMENT FROZEN BEFORE COORDINATOR CODE; NO POD** (2026-07-29) | Correct the literal single-family lease without changing the C6RSC3 interaction: one response-local arena and one proof repetition, with auxiliary admitted only after global challenge 8 | Leaf alone still defines the peak at **33,554,432 Fp2 / 536,870,912 B**.  At auxiliary admission the leaf has shrunk to **131,072 Fp2 / 2,097,152 B** and auxiliary occupies **393,216 Fp2 / 6,291,456 B**, for only **524,288 Fp2 / 8,388,608 B** combined.  The arena must reject early/duplicate family admission, a changed or second live repetition, a second arena, cap overflow, accounting underflow and persistence after completion.  Transcript order, four event replays, proof bytes, correlations, terminal claims, soundness and budget are unchanged; no implementation or timing credit is claimed. |
