...
| P7 report + GPU budget model | **A100 roofline + fused MAC + LogUp tree passed; rounds/PCS open** (2026-07-11) | report/PCG/cloud CPU anchors ✓; Goldilocks/F_p² roofline ✓; fused GEMM-MAC rho<=1.30 ✓; LogUp lookup-tree N=2^24 speedup gate ✓; LogUp round/fold, PCS/hash and proving-path integration open | Packed download remains 144.8 MB; required relative acceleration **5.48× prefill / 3.97× decode**. A100: roofline 55.48× stream / 300.94× chain; fused MAC `rho_kernel=1.003`; LogUp lookup tree CPU 189.3 ms -> GPU 2.864 ms = **66.12×**, every level exact. Sources: `p7-gpu-roofline-2026-07-11-a43d105.json`, `p7-gpu-fused-epilogue-2026-07-11-bde5d7d.json`, `p7-gpu-logup-tree-2026-07-11-5f7b443.json`. Next: LogUp sumcheck rounds/folds, then PCS row/global + blake3; final go/no-go remains open. |
