#!/usr/bin/env python3
"""Compile and run the P7 GPU LogUp lookup-side fraction-tree build."""
...
SOURCE = REPO / "cuda" / "p7_logup_tree.cu"
...
def unique_path(label: str, date: str, sha: str) -> Path:
    ...
    for i in range(1, 1000):
        ...
        if not candidate.exists():
            return candidate
    raise SystemExit("could not allocate append-only LogUp result path")
...
def main() -> int:
    ...
    is_dirty = dirty()
    if is_dirty:
        raise SystemExit("refusing LogUp GPU run from a dirty tracked tree")
    log2_n, gpu_reps, cpu_reps = (18, 3, 1) if args.quick else (24, 7, 3)

    with tempfile.TemporaryDirectory(prefix="volta-p7-logup-") as tmp:
        binary = Path(tmp) / "p7_logup_tree"
        compile_cmd = [
            args.nvcc,
            "-O3",
            "-std=c++17",
    ...
    report = {
        "milestone": "P7-gpu-logup-tree-quick" if args.quick else "P7-gpu-logup-tree",
        ...
        "kernel": kernel,
        "scope": {
            "leaf_side": "lookup / LeafP::Ones",
            "tree_build_only": True,
            "sumcheck_rounds_integrated": False,
            "proving_path_integrated": False,
        },
    }
    RESULTS.mkdir(parents=True, exist_ok=True)
    label = "p7-gpu-logup-tree-quick" if args.quick else "p7-gpu-logup-tree"
