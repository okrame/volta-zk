...
/// Provider result for one claimless C6.1 chain.  Both affine coordinates and
/// the base closure are public transcript derivatives, so the provider can
        ...
        O: ZkWhirOracleCommitter<F, EF, MT>,
    {
        ...
        for round in 0..config.n_rounds() {
            ...
            let mask_encoding_randomness: Vec<EF> =
                (0..mask_shape.randomness_len).map(|_| rng.random()).collect();
            let mask_codeword =
        ...
        let (base_case, base_case_closure) = base_prover.prove(
            self.dft,
...
fn replay_affine_batch<F, EF: p3_field::Field>(
    mut target: AffineClaim<EF>,
    data: &ZkSumcheckData<F, EF>,
    eps: EF,
    randomness: &Point<EF>,
) -> AffineClaim<EF> {
    target = target.prelude(eps, data.mu_tilde);
    for (wire, &gamma) in data.round_coefficients.iter().zip(randomness.iter()) {
        target = target.replay_round(wire, gamma);
    }
    target
}
