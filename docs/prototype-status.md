...
| C6 installed compiled-residual core | **FULL-T1 PAIRED ALGEBRA + MEMORY DIAGNOSTIC GREEN; CLEAN TWO-SEED RECORD / ORDINARY OVERHEAD PENDING** (2026-07-29) | Consuming strict decoder installation; local reverse workspace; role-map runtime values; transcript-streamed base-share coefficients; paired source/key folds | A dirty-tree full `100+50` diagnostic accepts both independent Δ coordinates over all **4,975,525** sources with provider/verifier linear-form equality and provider/client coefficient-binding equality; no coefficient vector is serialized.  The installed plan is **196,741,767 B** resident.  Each role retains **79,611,404 B** after compilation and uses a counted **541,141,500-B** residual-only compile peak including the temporary node workspace.  Informative local timings are **1.644425052 s** one-off install, **0.507563488 s** provider compile, **1.540541423 s** provider paired fold, **0.504332932 s** verifier compile and **1.501125329 s** diagnostic client fold.  The residual output itself is **64 B**, but this is not the final wrapper/certificate and earns no wire or timing credit.  Clean cross-seed records, bound ordinary-build overhead, wrapper, real-PCG session and hardware gates remain pending. |
