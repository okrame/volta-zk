...
canonical zero.  Typed pending-target replacement, `CacheSegK` removal, the resident
provider path, the 24-round transition, envelope/backend, local end-to-end and
...
| C6 Δ-residual inline certificate | **SCALED SOURCE-BOUND C6PS1 V2 → C6PC2 → 72-SLOT C6LNK2 → PAIRED PCS GREEN; FACTORIZED FOLD COMPILER + STRICT POST-RHO TRANSCRIPT GREEN; TYPED 24-ROUND INTEGRATION / CACHESEGK REMOVAL PENDING; NO POD** (2026-07-31) | Hide direct `auth_corrections` and Ligero `u_vectors`; one inline native-field wrapper; fixed-capacity committed KV cache; 17 accepts plus retry reserve; no deferred settlement | Q=121 keeps complete soundness at **79.472744138609180097... bits**.  The descendant has **pi_final 4,479,466 B** and complete response **33,656,098 B <=35,000,000 B**; known first exchange remains **146,058,504 B**.  `C6LNK2` is exact at **72 relations / six groups / 1,939,733 B per chain / 3,879,466 B paired PCS / 3,570 B link overhead**.  The amended cache event is **653 roots / 426,409 two-repetition numerator <2^19**.  Strict production core `C6PC2` is **3,506 B / 104 full correlations per tape**; strict `C6PS1` v2 remains **304 B / zero correlations**, for a **3,810-B** source-bound package.  Per repetition its fold K/V corrections follow the scalar root and precede the relation point; append K/V follows that point; predecessor owner is canonical zero.  The 6-round source-bound reference remains **1,506 B / 32 per tape / 64 pending claims** and replaces every former cache/cache-auxiliary stand-in through the real packed link/PCS fixture.  Full Lean is **3,261 jobs / 345 audited targets (106 C6)**; exact C6 budget tests are **9/9**.  Ordinary/trace PCS are **178/0/1** and **179/0/1**; ordinary/trace proto remain **143/0/1** and **151/0/1**.  The conservative pre-backend floor remains **11.1793342101 s**.  Typed runtime targets, the step-wise 24-round cache participant, resident provider, `CacheSegK` and response-field removal, measured timing and hardware gates remain pending. |
| C6 runtime cache-fold scalar batch | **FACTORIZED DENSE-DIFFERENTIAL + REAL 12+4 TWO-ROOT E2E + FORMAL CENSUS GREEN; C6PS1/24-ROUND INTEGRATION NEXT; NO POD** (2026-07-31) | Feature-only final-functional factors and opaque authenticated targets; canonical global scalar powers per repetition; no key/plaintext vector, dense `2^24` field or ordinary-build path change | The scaled `T=4` compiler matches an independent dense oracle on every K/V cell: **24 folds / 1,632 factors / 6,144 applications**.  The frozen `12+4` response compiles prover/verifier traces identically under two distinct roots at **576 / 44,928 / 516,096**; cross-root batch digests differ.  The cap is **576 records / 626,688 factor values**.  Compilation revalidates all geometry, record/aggregate digests, censuses, ordinals, 12-head cohorts and paired K/V families; changed factors/records/identity and incomplete families reject.  Lean proves successor-to-append transitivity, the 576 scalar-power bad-root bound and **653²=426,409<2^19**.  Audit **345 total /106 C6**, SHA-256 `007a24a1d31f9dde8a7484905803155298eaf00dc5d47e45e776ed3ff27881e6`.  Full ordinary/feature workspace suites, all-target checks, format and modified-file clippy filter are green.  Global clippy remains historically blocked outside modified scope.  The actual `C6PS1` codec/order is not migrated, the target is not yet a pending packed-link claim, `CacheSegK` remains live, and no wire/time/setup/hardware credit is claimed. |
