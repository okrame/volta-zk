...
`23,891,144` linear nodes.  Prover normalization now emits canonical digest
`0b4bb678...2f489bf` over `28,845,583` reachable value nodes, with
`23,874,732` reachable and `16,412` omitted raw operations.  Independent
verifier reconstruction is now exact at the same identity and reachable
count.  That v1 identity binds response-specific public/scalar values, so the
newly preregistered v2 seam must separate reusable topology from a
certificate-bound instance-value digest and pass a two-transcript-seed
differential plus setup-encoding census before it is admitted as the compiled
plan.  Cache constraint census, wrapper proof, local end-to-end and hardware
...
| C6 Δ-residual inline certificate | **LEAN AMPLIFICATION + PRODUCT-CLOSURE / RUST RESIDUAL-IR / HIDDEN-U / ROOFLINE / PAIRED CODEC+STATE / SOURCE CENSUS / PAIRED COMPLETE SOURCE WITNESS GREEN; INDEPENDENT EXACT-INSTANCE PLAN EXACT; PARAMETERIZED PLAN/CACHE/WRAPPER PENDING; HARD STOP BEFORE POD** (2026-07-29) | Hide direct `auth_corrections` and Ligero `u_vectors`; one inline native-field wrapper; fixed-capacity committed KV cache; 17 accepts plus retry reserve; no deferred settlement | Retained Q=121 response **29,176,632 B**; new-payload budget **5,823,368 B**; implemented `pi_final` roof **4,409,824 B** under the owner cap **4,500,000 B**; complete response roof **33,586,456 B <=35,000,000 B**.  Q=121 gives **79.472744138609166392... bits**, preserving the **78.80929487391641-bit** floor after four amplified wrapper events.  Two independent tapes each fit **17 accepts + 4 aborts**, and paired PCG setup is **76,742,930 B**.  Lean is **3,257 jobs / 274 total / 38 C6**, zero `sorry`/`admit`; schema-v2 setup/state/certificate codecs bind paired ranges, atomic abort/retry, anti-fork state and idempotent ACK recovery.  Fixture first exchange is **76,743,367 B**.  Hidden-`u` removes exactly **17,235,968 B**, binds **12,686** NTT/`ip` relations and emits a **1,912-B** prequery plus one **32-B** seed.  The exact source census is **4,975,525 leaves / 673 ProductClosures / 22,339 triples / 8,170 zero closures**.  The clean paired complete-source reference extracts all **4,975,525 `(r,d,m)` leaves per coordinate** with no second model run and unchanged allocation schedule; the 673 ProductMask plaintexts are independently refreshed.  The full prover trace reaches those censuses with **23,891,144 raw linear nodes** and no untracked operand; independent prover/verifier canonical v1 normalization emits **28,845,583 nodes**, **23,874,732 reachable operations each** and exact digest `0b4bb678...2f489bf`.  V1 binds response-specific scalar values and is therefore exact-instance evidence, not yet a reusable compiled plan.  The preregistered v2 seam separates a stable topology digest from a certificate-bound per-response instance digest and requires a two-seed full-T1 differential plus an exact setup-encoding census before cache/wrapper code.  Final bytes, real-PCG session, prover wall and hardware verdict remain pending. |
