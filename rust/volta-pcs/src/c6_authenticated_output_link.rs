...
use volta_proto::c6_residual::evaluate_c6_nbr2_coefficient_prefix_at_two_points;
...
use crate::c6_wrapper_pcs::{
    prove_c6_wrapper_pcs_assembled, prove_c6_wrapper_pcs_persisted_cuda_assembled,
    seal_authenticated_link_c6_wrapper_claims, verify_c6_wrapper_pcs_assembled,
...
const LINK_NBR2_BOUND_LABEL: &str = "c6_nbr2_correction_functional_bound";
...
const C63_SOURCE_FUNCTIONAL_CONTEXT: &str = "volta-zk/c63/residual-source-functionals/v1";
const C63_SOURCE_FUNCTIONAL_SCHEDULE_CONTEXT: &str =
    "volta-zk/c63/residual-source-functional-schedule/v1";
const LINK_C63_SOURCE_BOUND_LABEL: &str = "c63_residual_source_functionals_bound";
const C63_SOURCE_FUNCTIONAL_CORRECTIONS_LABEL: &str = "c63_residual_source_functional_corrections";
const C63_SOURCE_FUNCTIONAL_CORRELATION_BASE: u64 = 0x0C69_0000_0000_0000;
const C63_SOURCE_FUNCTIONAL_SLOTS: [u16; 2] = [3, 6];
...
pub const C63_AUTHENTICATED_SKETCH_SOURCE_LINK_RELATIONS: usize =
    C63_AUTHENTICATED_SKETCH_OUTPUT_LINK_PRODUCTION_RELATIONS + 3;
pub const C63_RESIDUAL_SOURCE_FUNCTIONAL_FRAME_BYTES: usize = 64;
...
/// Typed, response-local C6NBR2 statement. The coefficient prefix is public
/// verifier state derived from the installed compiler and never crosses the
/// certificate wire.
pub struct C6Nbr2CorrectionFunctional<'a> {
    ...
    coefficient_vector_digest: C6WrapperDigest,
    source_count: u32,
    source_log2: u8,
    correction: Fp2,
    coefficients: &'a [Fp2],
...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct C63ResidualSourceFunctionalsProvedLink {
    statement_digest: C6WrapperDigest,
}

#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct C63ResidualSourceFunctionalsVerifiedLink {
    statement_digest: C6WrapperDigest,
    terminal_m_keys: [VerifierKey; 2],
}
...
impl C63ResidualSourceFunctionalsProvedLink {
    pub fn statement_digest(self) -> C6WrapperDigest {
        self.statement_digest
    }
}
...
impl C63ResidualSourceFunctionalsVerifiedLink {
    ...
    pub fn terminal_m_keys(self) -> [VerifierKey; 2] {
        self.terminal_m_keys
    }
    ...
    pub(crate) fn for_test(
        ...
        Self { statement_digest, terminal_m_keys }
    }
}
...
impl fmt::Debug for C6Nbr2CorrectionFunctional<'_> {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("C6Nbr2CorrectionFunctional")
            .field("outer_statement_digest", &self.outer_statement_digest)
            ...
            .field("coefficient_vector_digest", &self.coefficient_vector_digest)
            .field("source_count", &self.source_count)
            .field("source_log2", &self.source_log2)
...
impl<'a> C6Nbr2CorrectionFunctional<'a> {
    ...
    pub fn new<B: C6ResidualCommitmentBinding>(
        ...
        if [
            statement.outer_statement_digest,
        ...
        .contains(&[0; 32])
            || statement.source_count == 0
        {
            return Err(C6AuthenticatedOutputLinkError::new(
                "C6NBR2 statement contains an empty binding",
            ));
        }
        statement.digest = c6_nbr2_statement_digest(&statement);
        Ok(statement)
    }

    pub fn digest(&self) -> C6WrapperDigest {
        self.digest
    }
    ...
    fn validate_for_link<B: C6ResidualCommitmentBinding>(
        ...
        if self.wrapper_statement_digest != fixed.residual_statement_digest()
            ...
            || self.digest != c6_nbr2_statement_digest(self)
        {
            return Err(C6AuthenticatedOutputLinkError::new(
                "C6NBR2 statement differs from the authenticated link",
            ));
        }
        Ok(())
    }
...
/// Two private linear openings of the residual correction columns. Column 3
/// contains authentication tape 0 and column 6 contains tape 1. The values
/// stay authenticated; only the public coefficient vectors and their digest
/// enter the link statement.
#[derive(Clone)]
pub struct C63ResidualSourceFunctionals {
    outer_statement_digest: C6WrapperDigest,
    wrapper_statement_digest: C6WrapperDigest,
    fixed_roots_digest: C6WrapperDigest,
    source_binding_digest: C6WrapperDigest,
    source_schedule_digest: C6WrapperDigest,
    functional_context_digest: C6WrapperDigest,
    coefficients: [Arc<[Fp2]>; 2],
    coefficient_digests: [C6WrapperDigest; 2],
    source_count: u32,
    source_log2: u8,
    digest: C6WrapperDigest,
}

impl fmt::Debug for C63ResidualSourceFunctionals {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("C63ResidualSourceFunctionals")
            .field("outer_statement_digest", &self.outer_statement_digest)
            .field("wrapper_statement_digest", &self.wrapper_statement_digest)
            .field("fixed_roots_digest", &self.fixed_roots_digest)
            .field("source_binding_digest", &self.source_binding_digest)
            .field("source_schedule_digest", &self.source_schedule_digest)
            .field("functional_context_digest", &self.functional_context_digest)
            .field("coefficient_digests", &self.coefficient_digests)
            .field("source_count", &self.source_count)
            .field("source_log2", &self.source_log2)
            .field("digest", &self.digest)
            .finish_non_exhaustive()
...
impl C63ResidualSourceFunctionals {
    ...
    fn new_inner<B: C6ResidualCommitmentBinding>(
        ...
        if [
            ...
            statement.source_binding_digest,
            statement.source_schedule_digest,
            statement.functional_context_digest,
        ]
        .contains(&[0; 32])
            || source_count == 0
            || statement.coefficients[1].len() != source_count as usize
        {
            return Err(C6AuthenticatedOutputLinkError::new(
                "C6.3 source-functional statement contains an empty binding",
            ));
        }
        statement.digest = c63_source_functionals_digest(&statement);
        statement.validate_for_link(fixed)?;
        Ok(statement)
    }
    ...
    pub fn source_schedule_digest(&self) -> C6WrapperDigest {
        self.source_schedule_digest
    }

    pub fn coefficients(&self, coordinate: usize) -> Result<&[Fp2]> {
        self.coefficients.get(coordinate).map(AsRef::as_ref).ok_or_else(|| {
            C6AuthenticatedOutputLinkError::new("C6.3 source-functional coordinate is invalid")
        })
    }
    ...
    fn validate_for_link<B: C6ResidualCommitmentBinding>(&self, fixed: &B) -> Result<()> {
        ...
        {
            return Err(C6AuthenticatedOutputLinkError::new(
                "C6.3 source-functional statement differs from the authenticated link",
            ));
        }
        Ok(())
    }
...
fn c63_source_functionals_digest(statement: &C63ResidualSourceFunctionals) -> C6WrapperDigest {
    let mut hasher = blake3::Hasher::new_derive_key(C63_SOURCE_FUNCTIONAL_CONTEXT);
    for digest in [
        statement.outer_statement_digest,
        statement.wrapper_statement_digest,
        statement.fixed_roots_digest,
        ...
        statement.source_schedule_digest,
        statement.functional_context_digest,
        statement.coefficient_digests[0],
        statement.coefficient_digests[1],
    ] {
        hasher.update(&digest);
    }
    for slot in C63_SOURCE_FUNCTIONAL_SLOTS {
        hasher.update(&slot.to_le_bytes());
    }
    hasher.update(&statement.source_count.to_le_bytes());
    hasher.update(&[statement.source_log2]);
    *hasher.finalize().as_bytes()
}

pub struct C63ResidualSourceFunctionalProverClaims {
    statement: C63ResidualSourceFunctionals,
    claims: [[ProverAuthed; C6_AUTHENTICATED_OUTPUT_LINK_TAPES]; 2],
}

impl C63ResidualSourceFunctionalProverClaims {
    pub fn new(
        statement: C63ResidualSourceFunctionals,
        claims: [[ProverAuthed; C6_AUTHENTICATED_OUTPUT_LINK_TAPES]; 2],
    ) -> Result<Self> {
        if claims.iter().any(|claim| claim[0].x != claim[1].x) {
            return Err(C6AuthenticatedOutputLinkError::new(
                "C6.3 source-functional prover tapes disagree on plaintext",
            ));
        }
        Ok(Self { statement, claims })
    }

    pub fn statement(&self) -> &C63ResidualSourceFunctionals {
        &self.statement
    }

    pub fn claims(&self) -> &[[ProverAuthed; C6_AUTHENTICATED_OUTPUT_LINK_TAPES]; 2] {
        &self.claims
    }
}

pub struct C63ResidualSourceFunctionalVerifierClaims {
    statement: C63ResidualSourceFunctionals,
    keys: [[VerifierKey; C6_AUTHENTICATED_OUTPUT_LINK_TAPES]; 2],
}

#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct C63ResidualSourceFunctionalFrame {
    corrections: [[Fp2; C6_AUTHENTICATED_OUTPUT_LINK_TAPES]; 2],
}

impl C63ResidualSourceFunctionalFrame {
    pub fn encode(self) -> [u8; C63_RESIDUAL_SOURCE_FUNCTIONAL_FRAME_BYTES] {
        let mut bytes = [0u8; C63_RESIDUAL_SOURCE_FUNCTIONAL_FRAME_BYTES];
        for (index, correction) in self.corrections.into_iter().flatten().enumerate() {
            let offset = index * 16;
            bytes[offset..offset + 8].copy_from_slice(&correction.c0.value().to_le_bytes());
            bytes[offset + 8..offset + 16].copy_from_slice(&correction.c1.value().to_le_bytes());
        }
        bytes
    }

    pub fn decode(bytes: &[u8]) -> Result<Self> {
        if bytes.len() != C63_RESIDUAL_SOURCE_FUNCTIONAL_FRAME_BYTES {
            return Err(C6AuthenticatedOutputLinkError::new(
                "C6.3 source-functional frame length differs",
            ));
        }
        let mut corrections = [[Fp2::ZERO; C6_AUTHENTICATED_OUTPUT_LINK_TAPES]; 2];
        for (index, correction) in corrections.iter_mut().flatten().enumerate() {
            let offset = index * 16;
            let c0 = u64::from_le_bytes(bytes[offset..offset + 8].try_into().expect("fixed limb"));
            let c1 =
                u64::from_le_bytes(bytes[offset + 8..offset + 16].try_into().expect("fixed limb"));
            if c0 >= P || c1 >= P {
                return Err(C6AuthenticatedOutputLinkError::new(
                    "C6.3 source-functional frame is noncanonical",
                ));
            }
            *correction = Fp2::new(Fp::new(c0), Fp::new(c1));
        }
        Ok(Self { corrections })
    }

    pub fn corrections(&self) -> &[[Fp2; C6_AUTHENTICATED_OUTPUT_LINK_TAPES]; 2] {
        &self.corrections
    }
}

pub fn authenticate_c63_residual_source_functionals(
    statement: C63ResidualSourceFunctionals,
    values: [Fp2; 2],
    streams: &mut [CorrelationStream; C6_AUTHENTICATED_OUTPUT_LINK_TAPES],
    transcript: &mut Transcript,
) -> Result<(C63ResidualSourceFunctionalProverClaims, C63ResidualSourceFunctionalFrame)> {
    let mut claims = [[ProverAuthed::ZERO; C6_AUTHENTICATED_OUTPUT_LINK_TAPES]; 2];
    let mut corrections = [[Fp2::ZERO; C6_AUTHENTICATED_OUTPUT_LINK_TAPES]; 2];
    for coordinate in 0..2 {
        for tape in 0..C6_AUTHENTICATED_OUTPUT_LINK_TAPES {
            let domain = c63_source_functional_correlation_domain(coordinate, tape)?;
            let correlation = streams[tape].draw_fulls(domain, 1)[0];
            streams[tape]
                .record_c6_fullfield_plaintexts(domain, &[values[coordinate]])
                .map_err(|error| C6AuthenticatedOutputLinkError::new(error.to_string()))?;
            corrections[coordinate][tape] = values[coordinate] - correlation.x;
            claims[coordinate][tape] = correlation.authenticate(values[coordinate]);
        }
    }
    let frame = C63ResidualSourceFunctionalFrame { corrections };
    transcript.append_message(C63_SOURCE_FUNCTIONAL_CORRECTIONS_LABEL, &frame.encode());
    Ok((C63ResidualSourceFunctionalProverClaims::new(statement, claims)?, frame))
}

pub fn verify_c63_residual_source_functionals(
    statement: C63ResidualSourceFunctionals,
    frame: &C63ResidualSourceFunctionalFrame,
    contexts: &mut [VerifierCtx; C6_AUTHENTICATED_OUTPUT_LINK_TAPES],
    transcript: &mut Transcript,
) -> Result<C63ResidualSourceFunctionalVerifierClaims> {
    let mut keys = [[VerifierKey::ZERO; C6_AUTHENTICATED_OUTPUT_LINK_TAPES]; 2];
    for coordinate in 0..2 {
        for tape in 0..C6_AUTHENTICATED_OUTPUT_LINK_TAPES {
            keys[coordinate][tape] = contexts[tape].correct_full_verifier_keys(
                c63_source_functional_correlation_domain(coordinate, tape)?,
                &[frame.corrections[coordinate][tape]],
            )[0];
        }
    }
    transcript.append_message(C63_SOURCE_FUNCTIONAL_CORRECTIONS_LABEL, &frame.encode());
    Ok(C63ResidualSourceFunctionalVerifierClaims::new(statement, keys))
}

impl C63ResidualSourceFunctionalVerifierClaims {
    pub fn new(
        statement: C63ResidualSourceFunctionals,
        keys: [[VerifierKey; C6_AUTHENTICATED_OUTPUT_LINK_TAPES]; 2],
    ) -> Self {
    ...
    pub fn keys(&self) -> &[[VerifierKey; C6_AUTHENTICATED_OUTPUT_LINK_TAPES]; 2] {
        &self.keys
    }
}

fn c6_nbr2_statement_digest(statement: &C6Nbr2CorrectionFunctional<'_>) -> C6WrapperDigest {
    let mut hasher = blake3::Hasher::new_derive_key(C6_NBR2_STATEMENT_CONTEXT);
    for digest in [
        statement.outer_statement_digest,
        statement.wrapper_statement_digest,
        statement.fixed_roots_digest,
        ...
        statement.native_profile_digest,
        statement.functional_digest,
        statement.coefficient_vector_digest,
    ] {
        hasher.update(&digest);
    }
    hasher.update(&C6_DELTA_RESIDUAL_COHORT_ID.to_le_bytes());
    hasher.update(&C6_NBR2_CORRECTION_SLOT.to_le_bytes());
    hasher.update(&[1, statement.source_log2]);
    hasher.update(&statement.source_count.to_le_bytes());
    hash_fp2(&mut hasher, statement.correction);
    *hasher.finalize().as_bytes()
}

fn c6_nbr2_coefficient_vector_digest(coefficients: &[Fp2]) -> C6WrapperDigest {
    let mut hasher = blake3::Hasher::new_derive_key("volta-zk/c6/nbr2-coefficient-vector/v1");
...
///
/// `values` is the complete committed tape-1 correction column: its lower
/// D23 source half and independent upper ZK half. `weights` is
...
#[derive(Clone, Copy)]
struct C6ResidualFunctionalAddition<'a> {
    weight: Fp2,
    coefficients: &'a [Fp2],
    value: Fp2,
}
...
impl C6Nbr2CombinedDelayedTerm {
    fn new(
        ...
        Self::new_with_additions(
            rho,
            values,
            target_point,
            &[C6ResidualFunctionalAddition {
                weight: gamma,
    ...
    fn new_with_additions(
        rho: Fp2,
        values: &[Fp2],
        target_point: &[Fp2],
        additions: &[C6ResidualFunctionalAddition<'_>],
        global_rounds: usize,
    ) -> Result<Self> {
        if values.len() < 2
            || !values.len().is_power_of_two()
            || target_point.len() != values.len().ilog2() as usize
            || target_point.last() != Some(&Fp2::ZERO)
            || additions.is_empty()
            || additions.iter().any(|addition| addition.coefficients.len() > values.len() / 2)
            || global_rounds != target_point.len() + 1
        {
            return Err(C6AuthenticatedOutputLinkError::new(
                "C6 combined residual-functional term geometry mismatch",
            ));
        }
        let source_point = &target_point[..target_point.len() - 1];
        let (values, zk_values) = values.split_at(values.len() / 2);
        for addition in additions {
            let actual = values
                .iter()
                .zip(addition.coefficients)
                .fold(Fp2::ZERO, |sum, (&value, &coefficient)| sum + value * coefficient);
            if actual != addition.value {
                return Err(C6AuthenticatedOutputLinkError::new(
                    "C6 authenticated functional differs from its residual slot",
                ));
            }
        }
        let equality = eq_vec(source_point);
        let mut weights = equality.into_iter().map(|equality| rho * equality).collect::<Vec<_>>();
        for addition in additions {
            for (weight, &coefficient) in weights.iter_mut().zip(addition.coefficients.iter()) {
                *weight += addition.weight * coefficient;
            }
        }
        Ok(Self {
            values: values.to_vec(),
            zk_values: zk_values.to_vec(),
            weights,
    ...
    fn round_values(&self) -> Result<(Fp2, Fp2)> {
        if self.values.len() != self.weights.len()
            || self.values.len() != self.zk_values.len()
        ...
        if self.trailing_virtual_rounds > 0 {
            let at_zero = self.values[0] * self.weights[0] * self.virtual_factor;
            let value_at_two = self.zk_values[0] + self.zk_values[0] - self.values[0];
            return Ok((at_zero, Fp2::ZERO - value_at_two * self.weights[0] * self.virtual_factor));
        }
    ...
    fn bind(&mut self, challenge: Fp2) -> Result<()> {
        if self.leading_virtual_rounds > 0 {
            ...
            fold_low(&mut self.zk_values, challenge);
            fold_low(&mut self.weights, challenge);
        } else if self.trailing_virtual_rounds > 0 {
            self.values[0] = self.values[0] + (self.zk_values[0] - self.values[0]) * challenge;
            self.zk_values[0] = self.values[0];
            self.virtual_factor = self.virtual_factor * (Fp2::ONE - challenge);
...
enum C6ReferenceDelayedTerm {
    Equality(DelayedTerm),
    Nbr2(C6Nbr2CombinedDelayedTerm),
}

impl C6ReferenceDelayedTerm {
    fn active_sum(&self) -> Fp2 {
        match self {
            Self::Equality(term) => term.active_sum(),
            Self::Nbr2(term) => term.active_sum(),
        }
    }

    fn round_values(&self) -> Result<(Fp2, Fp2)> {
        match self {
            Self::Equality(term) => term.round_values(),
            Self::Nbr2(term) => term.round_values(),
        }
    }
    ...
    fn bind(&mut self, challenge: Fp2) -> Result<()> {
        match self {
            ...
            Self::Nbr2(term) => term.bind(challenge),
        }
    }

    fn terminal(&self) -> Result<Fp2> {
        match self {
            Self::Equality(term) => term.terminal(),
            Self::Nbr2(term) => term.terminal(),
        }
    }
}
...
struct C6CudaPersistedCoefficientDelayedTerm {
    coefficient: Fp2,
    claimed_value: Fp2,
    owner: Option<C6CudaPersistedLinkFoldOwner>,
    target_point: Vec<Fp2>,
    leading_virtual_rounds: usize,
    virtual_factor: Fp2,
    global_round: u16,
...
struct C6CudaNbr2CombinedDelayedTerm {
    initial_value: Fp2,
    values: Option<C6CudaPersistedLinkFoldOwner>,
    weights: Option<C6CudaPersistedLinkFoldOwner>,
    leading_virtual_rounds: usize,
    trailing_virtual_rounds: usize,
    virtual_factor: Fp2,
    global_round: u16,
...
fn build_cuda_residual_functional_weights(
    backend: &mut Backend,
    weight_len: usize,
    source_point: &[Fp2],
    equality_weight: Fp2,
    addition: C6ResidualFunctionalAddition<'_>,
    metrics: &mut C6PersistedLinkFoldMetrics,
) -> Result<DeviceBuffer<Fp2Repr>> {
    let weights = backend.alloc_device::<Fp2Repr>(weight_len).map_err(|error| {
        C6AuthenticatedOutputLinkError::new(format!(
            "allocate C6 CUDA residual-functional weights: {error}"
        ))
    })?;
    let mut start = 0usize;
    while start < addition.coefficients.len() {
        let count = (addition.coefficients.len() - start).min(16 * 1024);
        let raw = addition.coefficients[start..start + count]
            .iter()
            .copied()
            .map(Fp2Repr::from)
            .collect::<Vec<_>>();
        if let Err(error) = backend.upload_device(&weights, start, &raw) {
            let _ = backend.free_device(weights);
            return Err(C6AuthenticatedOutputLinkError::new(format!(
                "upload C6 CUDA residual-functional coefficients: {error}"
            )));
        }
        metrics.cuda_h2d_bytes =
            metrics.cuda_h2d_bytes.checked_add((count * 16) as u64).ok_or_else(|| {
                C6AuthenticatedOutputLinkError::new("C6 residual-functional H2D overflow")
            })?;
        start += count;
    }
    if let Err(error) = backend.fp2_affine_eq_weights_inplace_device(
        &weights,
        addition.coefficients.len(),
        source_point,
        equality_weight,
        addition.weight,
    ) {
        let _ = backend.free_device(weights);
        return Err(C6AuthenticatedOutputLinkError::new(format!(
            "build C6 CUDA residual-functional weights: {error}"
        )));
    }
    metrics.cuda_kernel_calls = metrics.cuda_kernel_calls.checked_add(1).ok_or_else(|| {
        C6AuthenticatedOutputLinkError::new("C6 residual-functional kernel overflow")
    })?;
    metrics.cuda_h2d_bytes =
        metrics.cuda_h2d_bytes.checked_add((source_point.len() * 16) as u64).ok_or_else(|| {
            C6AuthenticatedOutputLinkError::new("C6 residual-functional H2D overflow")
        })?;
    Ok(weights)
}

impl C6CudaNbr2CombinedDelayedTerm {
    #[allow(clippy::too_many_arguments)]
    fn new(
        rho: Fp2,
        gamma: Fp2,
        ...
        let addition = C6ResidualFunctionalAddition {
            weight: gamma,
            coefficients: nbr2.coefficients(),
            value: nbr2.correction(),
        };
        Self::new_with_additions(
            rho,
            claimed_value,
            ...
            &[addition],
            nbr2.digest(),
            c6_nbr2_persisted_term_digest(descriptor, rho, gamma, nbr2.digest()),
            backend,
            spill_root,
            global_rounds,
            metrics,
    ...
    fn new_with_additions(
        ...
        if descriptor.cohort_id != C6_DELTA_RESIDUAL_COHORT_ID
            || !C63_SOURCE_FUNCTIONAL_SLOTS.contains(&descriptor.slot)
            || descriptor.target_point.last() != Some(&Fp2::ZERO)
            || source.coefficient_len()
                != 1usize.checked_shl(descriptor.target_point.len() as u32).unwrap_or_default()
            || global_rounds != descriptor.target_point.len() + 1
            || additions.is_empty()
            || [functional_digest, target_digest].contains(&[0; 32])
        {
            return Err(C6AuthenticatedOutputLinkError::new(
                "C6 CUDA residual-functional term geometry mismatch",
            ));
        }
        let (statement_digest, session_digest, root) = source.binding();
        if statement_digest != descriptor.wrapper_statement_digest {
            return Err(C6AuthenticatedOutputLinkError::new(
                "C6NBR2 CUDA residual statement mismatch",
            ));
        }
        let (values, initial_metrics) = source.open_cuda_link_fold_owner(
            backend,
            spill_root,
            descriptor.repetition,
            descriptor.cohort_id,
        ...
        if values.binding().root != root {
            return Err(C6AuthenticatedOutputLinkError::new("C6NBR2 CUDA residual root mismatch"));
        }
        metrics.absorb_nonlive(initial_metrics)?;
        let values = values.into_boolean_evaluations(backend, metrics)?;
        let source_point = &descriptor.target_point[..descriptor.target_point.len() - 1];
        let weight_len = 1usize.checked_shl(source_point.len() as u32).unwrap_or_default();
        if additions.iter().any(|addition| addition.coefficients.len() > weight_len) {
            return Err(C6AuthenticatedOutputLinkError::new(
                "C6 CUDA residual functional exceeds source domain",
            ));
        }
        let first = additions[0];
        let public_weights = build_cuda_residual_functional_weights(
            backend,
            weight_len,
            source_point,
            rho,
        ...
        for addition in &additions[1..] {
            let extra = match build_cuda_residual_functional_weights(
                backend,
                weight_len,
                source_point,
                Fp2::ZERO,
                *addition,
                ...
                Ok(extra) => extra,
                Err(error) => {
                    let _ = backend.free_device(public_weights);
                    return Err(error);
                }
            };
            if let Err(error) =
                backend.fp2_add_inplace_device(&public_weights, 0, &extra, 0, weight_len)
            {
                let _ = backend.free_device(extra);
                let _ = backend.free_device(public_weights);
                return Err(C6AuthenticatedOutputLinkError::new(format!(
                    "combine C6 CUDA residual-functional weights: {error}"
                )));
            }
            metrics.cuda_kernel_calls =
                metrics.cuda_kernel_calls.checked_add(1).ok_or_else(|| {
                    C6AuthenticatedOutputLinkError::new("C6 residual-functional kernel overflow")
                })?;
            if let Err(error) = backend.free_device(extra) {
                let _ = backend.free_device(public_weights);
                return Err(C6AuthenticatedOutputLinkError::new(format!(
                    "release C6 CUDA residual-functional weights: {error}"
                )));
            }
        }
        let weights = C6CudaPersistedLinkFoldOwner::open_public_evaluation_device_owner(
            backend,
            spill_root,
            statement_digest,
            session_digest,
        ...
        let initial_value = additions
            .iter()
            .fold(rho * claimed_value, |sum, addition| sum + addition.weight * addition.value);
        if values.boolean_lower_dot(&weights, backend, metrics)? != initial_value {
            return Err(C6AuthenticatedOutputLinkError::new(
                "C6 CUDA authenticated functional differs from its residual slot",
            ));
        }
        Ok(Self {
            initial_value,
    ...
    fn round_values(
        ...
        if values.binding().round != self.global_round
            || weights.binding().round != self.global_round
        {
            return Err(C6AuthenticatedOutputLinkError::new(
                "C6NBR2 CUDA combined-term round mismatch",
            ));
        }
        if self.leading_virtual_rounds > 0 {
            let at_zero = self.initial_value * self.virtual_factor;
            return Ok((at_zero, Fp2::ZERO - at_zero));
        }
        let endpoints = if weights.coefficient_len() > 1 {
            values.boolean_lower_product_round(weights, backend, metrics)?
        } else if self.trailing_virtual_rounds > 0 {
            values.boolean_z_endpoints(weights, backend, metrics)?
        } else {
            return Err(C6AuthenticatedOutputLinkError::new(
                "C6NBR2 CUDA combined term has no active round",
            ));
        };
        Ok((endpoints.0 * self.virtual_factor, endpoints.1 * self.virtual_factor))
    }
    ...
    fn bind(
        ...
        let next_round = self
            .global_round
            .checked_add(1)
            .ok_or_else(|| C6AuthenticatedOutputLinkError::new("C6NBR2 CUDA round overflow"))?;
        let values = self.values.take().ok_or_else(|| {
            C6AuthenticatedOutputLinkError::new("C6NBR2 CUDA residual owner consumed")
        })?;
        let weights = self.weights.take().ok_or_else(|| {
            C6AuthenticatedOutputLinkError::new("C6NBR2 CUDA public owner consumed")
        })?;
        let (values, weights) = if self.leading_virtual_rounds > 0 {
            self.virtual_factor = self.virtual_factor * (Fp2::ONE - challenge);
            self.leading_virtual_rounds -= 1;
            (
                values.advance_virtual_create_new(challenge, next_round, metrics)?,
                weights.advance_virtual_create_new(challenge, next_round, metrics)?,
            )
        } else if weights.coefficient_len() > 1 {
            (
                values.bind_boolean_rows_create_new(backend, challenge, next_round, 2, metrics)?,
                weights.bind_boolean_rows_create_new(backend, challenge, next_round, 1, metrics)?,
            )
        } else if self.trailing_virtual_rounds > 0 {
            self.virtual_factor = self.virtual_factor * (Fp2::ONE - challenge);
            self.trailing_virtual_rounds -= 1;
            (
                values.bind_boolean_rows_create_new(backend, challenge, next_round, 1, metrics)?,
                weights.advance_virtual_create_new(challenge, next_round, metrics)?,
            )
        } else {
            return Err(C6AuthenticatedOutputLinkError::new(
                "C6NBR2 CUDA combined term is already terminal",
            ));
        };
    ...
    fn terminal(
        ...
        if self.leading_virtual_rounds != 0
            || self.trailing_virtual_rounds != 0
            || values.coefficient_len() != 1
            || weights.coefficient_len() != 1
            || values.binding().round != self.global_round
            || weights.binding().round != self.global_round
        {
            return Err(C6AuthenticatedOutputLinkError::new(
                "invalid C6NBR2 CUDA combined terminal state",
            ));
        }
        Ok(values.terminal(backend, metrics)?
            * weights.terminal(backend, metrics)?
            * self.virtual_factor)
    }
    ...
    fn release(
        ...
        self.values
            .take()
            .ok_or_else(|| C6AuthenticatedOutputLinkError::new("C6NBR2 CUDA residual missing"))?
            .release(backend, metrics)?;
        self.weights
            .take()
            .ok_or_else(|| C6AuthenticatedOutputLinkError::new("C6NBR2 CUDA weights missing"))?
            .release(backend, metrics)?;
        Ok(())
    }

    fn values(&self) -> Result<&C6CudaPersistedLinkFoldOwner> {
        self.values.as_ref().ok_or_else(|| {
            C6AuthenticatedOutputLinkError::new("C6NBR2 CUDA residual owner missing")
        })
    }

    fn weights(&self) -> Result<&C6CudaPersistedLinkFoldOwner> {
        self.weights
            .as_ref()
            .ok_or_else(|| C6AuthenticatedOutputLinkError::new("C6NBR2 CUDA public owner missing"))
    }
}

impl C6CudaPersistedCoefficientDelayedTerm {
    #[allow(clippy::too_many_arguments)]
    fn new(
        coefficient: Fp2,
        claimed_value: Fp2,
        ...
        if owner.binding().root != root {
            return Err(C6AuthenticatedOutputLinkError::new(
                "C6 CUDA coefficient link root mismatch",
            ));
        }
        metrics.absorb_nonlive(initial_metrics)?;
        Ok(Self {
    ...
    fn initial_value(&self) -> Fp2 {
        self.coefficient * self.claimed_value
    }
    ...
    fn round_values(
        ...
        {
            return Err(C6AuthenticatedOutputLinkError::new(
                "invalid C6 CUDA coefficient link term state",
            ));
        }
        if self.leading_virtual_rounds > 0 {
            let at_zero = self.coefficient * self.claimed_value * self.virtual_factor;
            return Ok((at_zero, Fp2::ZERO - at_zero));
        }
        if owner.coefficient_len() < 2 || self.target_point.is_empty() {
            return Err(C6AuthenticatedOutputLinkError::new("C6 CUDA link has no active round"));
        }
        let target = self.target_point[0];
        let (value_zero, value_two) =
            owner.round_endpoints(backend, &self.target_point[1..], metrics)?;
        let equality_zero = Fp2::ONE - target;
        let equality_two = target + target - equality_zero;
        Ok((
            self.coefficient * value_zero * equality_zero * self.virtual_factor,
    ...
    fn bind(
        ...
        let next_round = self
            .global_round
            .checked_add(1)
            .ok_or_else(|| C6AuthenticatedOutputLinkError::new("C6 CUDA link round overflow"))?;
        let owner = self.owner.take().ok_or_else(|| {
            C6AuthenticatedOutputLinkError::new("C6 CUDA link owner already consumed")
        })?;
        let successor = if self.leading_virtual_rounds > 0 {
            ...
            let target = *self.target_point.first().ok_or_else(|| {
                C6AuthenticatedOutputLinkError::new("C6 CUDA link target exhausted")
            })?;
            self.virtual_factor = self.virtual_factor * eq_points(&[challenge], &[target]);
            self.target_point.remove(0);
            owner.bind_create_new(backend, challenge, next_round, metrics)?
        };
    ...
    fn terminal(
        ...
        {
            return Err(C6AuthenticatedOutputLinkError::new(
                "invalid C6 CUDA coefficient link terminal state",
            ));
        }
        Ok(self.coefficient * owner.terminal(backend, metrics)? * self.virtual_factor)
    }
    ...
    fn release(
        ...
        self.owner
            .take()
            .ok_or_else(|| {
                C6AuthenticatedOutputLinkError::new("C6 CUDA link owner already released")
            })?
            .release(backend, metrics)?;
        Ok(())
    }

    fn owner(&self) -> Result<&C6CudaPersistedLinkFoldOwner> {
        self.owner
            .as_ref()
            .ok_or_else(|| C6AuthenticatedOutputLinkError::new("C6 CUDA link owner missing"))
    }
}

enum C6CudaPersistedDelayedTerm {
    Equality(C6CudaPersistedCoefficientDelayedTerm),
    Nbr2(C6CudaNbr2CombinedDelayedTerm),
}

impl C6CudaPersistedDelayedTerm {
    fn initial_value(&self) -> Fp2 {
        match self {
            Self::Equality(term) => term.initial_value(),
            Self::Nbr2(term) => term.initial_value(),
        }
    }
    ...
    fn round_values(
        ...
        match self {
            Self::Equality(term) => term.round_values(backend, metrics),
            Self::Nbr2(term) => term.round_values(backend, metrics),
        }
    }
    ...
    fn bind(
        ...
        match self {
            Self::Equality(term) => term.bind(backend, challenge, metrics),
            Self::Nbr2(term) => term.bind(backend, challenge, metrics),
        }
    }
    ...
    fn terminal(
        ...
        match self {
            Self::Equality(term) => term.terminal(backend, metrics),
            Self::Nbr2(term) => term.terminal(backend, metrics),
        }
    }
    ...
    fn release(
        ...
        match self {
            Self::Equality(term) => term.release(backend, metrics),
            Self::Nbr2(term) => term.release(backend, metrics),
        }
    }
}
...
fn c6_nbr2_persisted_term_digest(
    descriptor: &C6PendingSlotDescriptor,
    rho: Fp2,
    gamma: Fp2,
    nbr2_statement_digest: C6WrapperDigest,
) -> C6WrapperDigest {
    let mut hasher = blake3::Hasher::new_derive_key("volta-zk/c6/nbr2-persisted-term/v1");
    hasher.update(&persisted_link_term_digest(descriptor, rho));
    hasher.update(&nbr2_statement_digest);
    hash_fp2(&mut hasher, gamma);
    *hasher.finalize().as_bytes()
}

fn c63_residual_functional_persisted_term_digest(
    descriptor: &C6PendingSlotDescriptor,
    rho: Fp2,
    additions: &[(Fp2, C6WrapperDigest)],
) -> C6WrapperDigest {
    let mut hasher =
        blake3::Hasher::new_derive_key("volta-zk/c63/residual-functional-persisted-term/v1");
    hasher.update(&persisted_link_term_digest(descriptor, rho));
    hasher.update(&[additions.len() as u8]);
    for (weight, statement_digest) in additions {
...
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct C6ProductionAuthenticatedOutputLinkMetrics {
    pub link: C6AuthenticatedOutputLinkMetrics,
    pub fold_state: C6PersistedLinkFoldMetrics,
    pub wrapper_pcs: C6ProductionWrapperPcsMetrics,
    pub attempt_directories_created: u64,
    pub attempt_fsync_count: u64,
}
...
#[allow(dead_code)]
struct C6PersistedLinkRoundOutput {
    point: Vec<Fp2>,
    final_claims: [ProverAuthed; C6_AUTHENTICATED_OUTPUT_LINK_TAPES],
    corrections: Vec<[[Fp2; 2]; C6_AUTHENTICATED_OUTPUT_LINK_TAPES]>,
    terminals: Vec<Fp2>,
...
#[allow(dead_code)]
struct C6PersistedLinkPreparation {
    repetition_proofs: Vec<C6LinkRepetitionProof>,
    assembled: C6AssembledWrapperClaims,
    final_claims: [[ProverAuthed; C6_AUTHENTICATED_OUTPUT_LINK_TAPES]; C6_WRAPPER_REPETITIONS],
    metrics: C6PersistedLinkFoldMetrics,
}

#[allow(clippy::too_many_arguments)]
#[allow(dead_code)]
fn prepare_c6_authenticated_output_link_persisted(
    ...
    validate_persisted_prover_cohorts(fixed, cohorts, session_digest)?;
    let readers = cohorts
        .iter()
        .map(C6PersistedWrapperCohort::open_coefficient_slots)
    ...
    for repetition in 0..C6_WRAPPER_REPETITIONS {
        ...
        for (entry, rho) in entries.iter().zip(&rhos) {
            let cohort_index = fixed
                .commitments()
                .iter()
                .position(|commitment| commitment.spec.cohort_id == entry.descriptor.cohort_id)
                .ok_or_else(|| {
                    C6AuthenticatedOutputLinkError::new("missing C6 persisted link cohort")
                })?;
            terms.push(C6PersistedCoefficientDelayedTerm::new(
                *rho,
                &entry.descriptor,
                &readers[cohort_index],
                spill_root,
                rounds,
                &mut metrics,
            )?);
        }
        let initial_sum = terms.iter_mut().try_fold(Fp2::ZERO, |sum, term| {
            Ok::<_, C6AuthenticatedOutputLinkError>(sum + term.initial_value(&mut metrics)?)
        })?;
        if initial_sum != initial_claims[0].x {
            return Err(C6AuthenticatedOutputLinkError::new(
                "false C6 persisted link initial claim",
            ));
        }
        let round_output = prove_persisted_dual_tape_rounds(
            terms,
            initial_claims,
            repetition as u8,
            streams,
        ...
        for (entry, terminal) in entries.iter().zip(&round_output.terminals) {
            let cohort_index = fixed
                ...
                .position(|commitment| commitment.spec.cohort_id == entry.descriptor.cohort_id)
                .ok_or_else(|| {
                    C6AuthenticatedOutputLinkError::new("missing C6 persisted terminal cohort")
                })?;
            aggregates[cohort_index] += *terminal;
        }
        let (claims, recomputed_aggregates) = assemble_new_point_claims(
            fixed,
            repetition as u8,
            &entries,
            &rhos,
        ...
        if aggregates != recomputed_aggregates
            || round_output.final_claims.iter().any(|claim| {
                claim.x != aggregates.iter().copied().fold(Fp2::ZERO, |sum, value| sum + value)
            })
        {
            return Err(C6AuthenticatedOutputLinkError::new(
    ...
    if metrics.current_live_spill_bytes != 0 {
        return Err(C6AuthenticatedOutputLinkError::new(
            "C6 persisted link retained terminal spill state",
        ));
    }
    let assembled = seal_authenticated_link_c6_wrapper_claims(fixed, assembled_claims)?;
    Ok(C6PersistedLinkPreparation { repetition_proofs, assembled, final_claims, metrics })
}

#[allow(clippy::too_many_arguments)]
fn prepare_c6_authenticated_output_link_persisted_cuda(
    fixed: &C6FixedWrapperCommitments,
    cohorts: &[C6PersistedWrapperCohort],
    pending: &C6PendingSlotRegistryProver,
    nbr2: Option<&C6Nbr2CorrectionFunctional<'_>>,
    c63_source: Option<&C63ResidualSourceFunctionalProverClaims>,
    streams: &mut [CorrelationStream; C6_AUTHENTICATED_OUTPUT_LINK_TAPES],
    transcript: &mut Transcript,
    spill_root: &Path,
    session_digest: C6WrapperDigest,
    ...
    let (relations, rounds, cohort_count) = link_geometry(fixed)?;
    if backend.kind() != BackendKind::CudaResident || relations != expected_link_census(fixed).0 {
        return Err(C6AuthenticatedOutputLinkError::new(
    ...
    if let Some(source) = c63_source {
        source.statement().validate_for_link(fixed)?;
    }
    validate_prover_registry(fixed, pending)?;
    validate_persisted_prover_cohorts(fixed, cohorts, session_digest)?;
    let readers = cohorts
    ...
    for repetition in 0..C6_WRAPPER_REPETITIONS {
        ...
        let schedule = match nbr2 {
            Some(nbr2) => c6_nbr2_schedule_digest(schedule, nbr2.digest(), relations + 1),
            None => schedule,
        };
        schedule_digests.push(match c63_source {
            Some(source) => c63_source_functional_schedule_digest(
                schedule,
                source.statement().digest(),
                relations + usize::from(nbr2.is_some()) + 2,
            ),
            None => schedule,
        });
    }
    if nbr2.is_some() {
        transcript.append(LINK_NBR2_BOUND_LABEL, 0);
    }
    if c63_source.is_some() {
        transcript.append(LINK_C63_SOURCE_BOUND_LABEL, 0);
    }
    transcript.append(
        LINK_PREFIX_LABEL,
        LINK_HEADER_BYTES + C6_WRAPPER_REPETITIONS as u64 * LINK_REPETITION_PREFIX_BYTES,
    ...
    for repetition in 0..C6_WRAPPER_REPETITIONS {
        ...
        let rhos = scalar_power_weights(beta, entries.len());
        let additional =
            link_additional_weights(beta, entries.len(), nbr2.is_some(), c63_source.is_some());
        let mut initial_claims = array::from_fn(|tape| {
            entries
                .iter()
                .zip(&rhos)
                .fold(ProverAuthed::ZERO, |sum, (entry, rho)| sum.add(entry.auth[tape].scale(*rho)))
        ...
        if let Some(nbr2) = nbr2 {
            let public = ProverAuthed::from_public(nbr2.correction())
                .scale(additional.nbr2.expect("C6NBR2 weight"));
            for claim in &mut initial_claims {
                *claim = claim.add(public);
            }
        }
        if let Some(source) = c63_source {
            for (coordinate, &weight) in
                additional.c63.expect("C6.3 source-functional weights").iter().enumerate()
            {
                for tape in 0..C6_AUTHENTICATED_OUTPUT_LINK_TAPES {
                    initial_claims[tape] =
                        initial_claims[tape].add(source.claims()[coordinate][tape].scale(weight));
                }
            }
        }
        let mut terms = Vec::with_capacity(entries.len());
        for (entry, rho) in entries.iter().zip(&rhos) {
            let cohort_index = fixed
                ...
                .position(|commitment| commitment.spec.cohort_id == entry.descriptor.cohort_id)
                .ok_or_else(|| {
                    C6AuthenticatedOutputLinkError::new("missing C6 CUDA persisted link cohort")
                })?;
            let mut additions = Vec::with_capacity(2);
            let mut addition_bindings = Vec::with_capacity(2);
            if nbr2.is_some_and(|_| is_nbr2_correction_slot(&entry.descriptor)) {
                let nbr2 = nbr2.expect("checked C6NBR2 statement");
                let weight = additional.nbr2.expect("C6NBR2 weight");
                additions.push(C6ResidualFunctionalAddition {
                    weight,
                    coefficients: nbr2.coefficients(),
                    value: nbr2.correction(),
                });
                addition_bindings.push((weight, nbr2.digest()));
            }
            if let Some(source) = c63_source {
                if entry.descriptor.cohort_id == C6_DELTA_RESIDUAL_COHORT_ID {
                    if let Some(coordinate) = C63_SOURCE_FUNCTIONAL_SLOTS
                        .iter()
                        .position(|slot| *slot == entry.descriptor.slot)
                    {
                        let weight =
                            additional.c63.expect("C6.3 source-functional weights")[coordinate];
                        additions.push(C6ResidualFunctionalAddition {
                            weight,
                            coefficients: source.statement().coefficients(coordinate)?,
                            value: source.claims()[coordinate][0].x,
                        });
                        addition_bindings.push((weight, source.statement().digest()));
                    }
                }
            }
            if additions.is_empty() {
                terms.push(C6CudaPersistedDelayedTerm::Equality(
                    C6CudaPersistedCoefficientDelayedTerm::new(
                        *rho,
                        entry.auth[0].x,
                        &entry.descriptor,
                        &readers[cohort_index],
            ...
            } else if c63_source.is_some() {
                let digest = c63_residual_functional_persisted_term_digest(
                    &entry.descriptor,
                    *rho,
                    &addition_bindings,
                );
                terms.push(C6CudaPersistedDelayedTerm::Nbr2(
                    C6CudaNbr2CombinedDelayedTerm::new_with_additions(
                        *rho,
                        entry.auth[0].x,
                        &entry.descriptor,
                        &readers[cohort_index],
                ...
                terms.push(C6CudaPersistedDelayedTerm::Nbr2(C6CudaNbr2CombinedDelayedTerm::new(
                    *rho,
                    additional.nbr2.expect("C6NBR2 weight"),
                    entry.auth[0].x,
                    &entry.descriptor,
                    &readers[cohort_index],
                    nbr2.expect("checked C6NBR2 statement"),
                    backend,
                    spill_root,
                    rounds,
                    &mut metrics,
        ...
        let initial_sum = terms.iter().fold(Fp2::ZERO, |sum, term| sum + term.initial_value());
        if initial_sum != initial_claims[0].x {
            return Err(C6AuthenticatedOutputLinkError::new(
                "false C6 CUDA persisted link initial claim",
            ));
        }
        let round_output = prove_cuda_persisted_dual_tape_rounds(
            terms,
            initial_claims,
            repetition as u8,
            streams,
        ...
        for (entry, terminal) in entries.iter().zip(&round_output.terminals) {
            let cohort_index = fixed
                ...
                .position(|commitment| commitment.spec.cohort_id == entry.descriptor.cohort_id)
                .ok_or_else(|| {
                    C6AuthenticatedOutputLinkError::new("missing C6 CUDA persisted terminal cohort")
                })?;
            aggregates[cohort_index] += *terminal;
        }
        let mut residual_slot_weights = Vec::with_capacity(3);
        if let Some(gamma) = additional.nbr2 {
            residual_slot_weights.push((
                C6_NBR2_CORRECTION_SLOT,
                residual_functional_terminal_weight(
                    nbr2.expect("C6NBR2 statement").coefficients(),
                    gamma,
                    &round_output.point,
                )?,
            ));
        }
        if let (Some(source), Some(gammas)) = (c63_source, additional.c63) {
            for coordinate in 0..2 {
                residual_slot_weights.push((
                    C63_SOURCE_FUNCTIONAL_SLOTS[coordinate],
                    residual_functional_terminal_weight(
                        source.statement().coefficients(coordinate)?,
                        gammas[coordinate],
                        &round_output.point,
                    )?,
                ));
    ...
    if metrics.current_live_spill_bytes != 0 {
        return Err(C6AuthenticatedOutputLinkError::new(
            "C6 CUDA persisted link retained terminal spill state",
        ));
    }
    let assembled = seal_authenticated_link_c6_wrapper_claims(fixed, assembled_claims)?;
    Ok(C6PersistedLinkPreparation { repetition_proofs, assembled, final_claims, metrics })
...
pub fn prove_c6_authenticated_output_link_reference(
    ...
    prove_c6_authenticated_output_link_reference_inner(
        fixed,
        cohorts,
        pending,
        polynomials,
...
/// C6NBR2 scaled/reference prover. It adds one public correction claim while
/// preserving the exact C6LNK2 proof grammar and wrapper PCS opening census.
#[allow(clippy::too_many_arguments)]
pub fn prove_c6_authenticated_output_link_reference_nbr2(
    fixed: &C6FixedWrapperCommitments,
    cohorts: &[C6CommittedWrapperCohort],
    pending: C6PendingSlotRegistryProver,
    polynomials: &[C6LinkSlotPolynomial<'_>],
...
/// C6.3 reference link. The existing NBR2 relation and the two private
/// correction-column openings share the same residual slots and wrapper PCS.
#[allow(clippy::too_many_arguments)]
pub fn prove_c63_authenticated_output_link_reference_nbr2_strict(
    fixed: &C6FixedWrapperCommitments,
    cohorts: &[C6CommittedWrapperCohort],
    pending: C6PendingSlotRegistryProver,
    polynomials: &[C6LinkSlotPolynomial<'_>],
    nbr2: &C6Nbr2CorrectionFunctional<'_>,
    source_functionals: &C63ResidualSourceFunctionalProverClaims,
    streams: &mut [CorrelationStream; C6_AUTHENTICATED_OUTPUT_LINK_TAPES],
    transcript: &mut Transcript,
) -> Result<(
    C6AuthenticatedOutputLinkProof,
    ...
    C6AuthenticatedOutputLinkMetrics,
    C6Nbr2ProvedLink,
    C63ResidualSourceFunctionalsProvedLink,
)> {
    let (proof, bound, metrics) = prove_c6_authenticated_output_link_reference_inner(
        fixed,
        cohorts,
        pending,
        polynomials,
        Some(nbr2),
        Some(source_functionals),
        streams,
        transcript,
    )?;
    Ok((
        ...
        bound,
        metrics,
        C6Nbr2ProvedLink { statement_digest: nbr2.digest() },
        C63ResidualSourceFunctionalsProvedLink {
            statement_digest: source_functionals.statement().digest(),
        },
    ))
}
...
fn prove_c6_authenticated_output_link_reference_inner(
    ...
    if let Some(c63_source) = c63_source {
        c63_source.statement().validate_for_link(fixed)?;
    }
    validate_prover_registry(fixed, &pending)?;
    ...
    for repetition in 0..C6_WRAPPER_REPETITIONS {
        ...
        for (entry, rho) in entries.iter().zip(&rhos) {
            ...
            if nbr2.is_some_and(|_| is_nbr2_correction_slot(&entry.descriptor)) {
                let nbr2 = nbr2.expect("checked C6NBR2 statement");
                additions.push(C6ResidualFunctionalAddition {
                    weight: additional.nbr2.expect("C6NBR2 weight"),
                    coefficients: nbr2.coefficients(),
                    value: nbr2.correction(),
                });
            }
            if let Some(source) = c63_source {
                if entry.descriptor.cohort_id == C6_DELTA_RESIDUAL_COHORT_ID {
                    ...
                    {
                        additions.push(C6ResidualFunctionalAddition {
                            weight: additional.c63.expect("C6.3 source-functional weights")
                                [coordinate],
                            coefficients: source.statement().coefficients(coordinate)?,
                            value: source.claims()[coordinate][0].x,
                        });
            ...
            if additions.is_empty() {
                terms.push(C6ReferenceDelayedTerm::Equality(DelayedTerm::new(
                    *rho,
                    polynomial,
                    &entry.descriptor.target_point,
                    rounds,
                ...
                terms.push(C6ReferenceDelayedTerm::Nbr2(
                    C6Nbr2CombinedDelayedTerm::new_with_additions(
                        *rho,
                        polynomial,
                        &entry.descriptor.target_point,
                        &additions,
    ...
    let metrics = C6AuthenticatedOutputLinkMetrics {
        relations_per_repetition: (relations
            + usize::from(nbr2.is_some())
            + 2 * usize::from(c63_source.is_some())) as u64,
...
/// Production C6LNK2 prover. This is the sole entry point which converts the
/// 72 pending authenticated slot relations per repetition into claims sealed
/// for the persisted/CUDA two-chain PCS.
#[allow(clippy::too_many_arguments)]
pub fn prove_c6_authenticated_output_link_persisted_cuda(
    fixed: &C6FixedWrapperCommitments,
    cohorts: &[C6PersistedWrapperCohort],
    pending: C6PendingSlotRegistryProver,
    streams: &mut [CorrelationStream; C6_AUTHENTICATED_OUTPUT_LINK_TAPES],
    ...
    prove_c6_authenticated_output_link_persisted_cuda_inner(
        fixed,
        cohorts,
        pending,
        None,
...
/// Production C6NBR2 prover. The additional correction relation reuses the
/// exact C6LNK2 wire grammar and the committed residual slot-6 opening.
#[allow(clippy::too_many_arguments)]
pub fn prove_c6_authenticated_output_link_persisted_cuda_nbr2(
    fixed: &C6FixedWrapperCommitments,
    cohorts: &[C6PersistedWrapperCohort],
    pending: C6PendingSlotRegistryProver,
    nbr2: &C6Nbr2CorrectionFunctional<'_>,
...
/// Production C6.3 link. The two authenticated source functionals are folded
/// into residual slots 3 and 6 without adding a PCS body or changing C6LNK2.
#[allow(clippy::too_many_arguments)]
pub fn prove_c63_authenticated_output_link_persisted_cuda_nbr2_strict(
    fixed: &C6FixedWrapperCommitments,
    cohorts: &[C6PersistedWrapperCohort],
    pending: C6PendingSlotRegistryProver,
    nbr2: &C6Nbr2CorrectionFunctional<'_>,
    ...
    if !fixed.is_c63_authenticated_sketch_profile() {
        return Err(C6AuthenticatedOutputLinkError::new(
            "C6.3 production link requires authenticated-sketch roots",
        ));
    }
    let (proof, bound, metrics) = prove_c6_authenticated_output_link_persisted_cuda_inner(
        fixed,
        cohorts,
        pending,
        Some(nbr2),
...
fn prove_c6_authenticated_output_link_persisted_cuda_inner(
    ...
    if backend.kind() != BackendKind::CudaResident {
        return Err(C6AuthenticatedOutputLinkError::new(
            "C6 production persisted link requires cuda-resident backend",
        ));
    }
    if !fixed.is_production_profile() || session_digest == [0; 32] {
        return Err(C6AuthenticatedOutputLinkError::new(
            "C6 production persisted link statement/session mismatch",
        ));
    }
    let spill_root = spill_root.as_ref();
    if !spill_root.is_dir() {
        return Err(C6AuthenticatedOutputLinkError::new(
            "C6 production persisted link spill root is not a directory",
        ));
    }
    validate_prover_registry(fixed, &pending)?;
    validate_persisted_prover_cohorts(fixed, cohorts, session_digest)?;
    let link_spill_root = spill_root.join("c6lnk2-folds");
    let pcs_spill_root = spill_root.join("c6lnk2-wrapper-pcs");
    fs::create_dir(&link_spill_root).map_err(|error| {
        C6AuthenticatedOutputLinkError::new(format!("create C6 production link fold root: {error}"))
    })?;
    fs::create_dir(&pcs_spill_root).map_err(|error| {
        C6AuthenticatedOutputLinkError::new(format!("create C6 production link PCS root: {error}"))
    })?;
    File::open(spill_root).and_then(|root| root.sync_all()).map_err(|error| {
        C6AuthenticatedOutputLinkError::new(format!("fsync C6 production link spill root: {error}"))
    })?;

    let preparation = prepare_c6_authenticated_output_link_persisted_cuda(
        fixed,
        cohorts,
        &pending,
        nbr2,
    ...
    let C6PersistedLinkPreparation {
        repetition_proofs,
        assembled,
        final_claims,
        metrics: fold_state,
    } = preparation;
    let (wrapper_pcs, wrapper_pcs_metrics) = prove_c6_wrapper_pcs_persisted_cuda_assembled(
    ...
    for repetition in 0..C6_WRAPPER_REPETITIONS {
        ...
        for tape in 0..C6_AUTHENTICATED_OUTPUT_LINK_TAPES {
            ...
            if residual.x != Fp2::ZERO {
                return Err(C6AuthenticatedOutputLinkError::new(
                    "nonzero C6 production link terminal residual",
                ));
            }
            terminal_tags[repetition][tape] = zero_open_prover(&residual, transcript);
        }
    ...
    let link = C6AuthenticatedOutputLinkMetrics {
        relations_per_repetition: (base_relations
            + usize::from(nbr2.is_some())
            + 2 * usize::from(c63_source.is_some())) as u64,
    ...
    {
        return Err(C6AuthenticatedOutputLinkError::new(
            "C6 production persisted link exact census changed",
        ));
    }
    let bound = C6BoundSlotRegistryProver {
        entries: pending
    ...
    Ok((
        ...
        C6ProductionAuthenticatedOutputLinkMetrics {
            link,
            fold_state,
            wrapper_pcs: wrapper_pcs_metrics,
            attempt_directories_created: 2,
            attempt_fsync_count: 1,
        },
    ))
}
...
pub fn verify_c6_authenticated_output_link_reference(
    ...
    refuse_production_reference(fixed)?;
    verify_c6_authenticated_output_link_inner(
        fixed, pending, proof, None, None, contexts, transcript,
    )
}

pub fn verify_c6_authenticated_output_link_reference_nbr2(
    fixed: &C6FixedWrapperCommitments,
    pending: C6PendingSlotRegistryVerifier,
    proof: &C6AuthenticatedOutputLinkProof,
    nbr2: &C6Nbr2CorrectionFunctional<'_>,
...
pub fn verify_c63_authenticated_output_link_reference_nbr2_strict(
    fixed: &C6FixedWrapperCommitments,
    pending: C6PendingSlotRegistryVerifier,
    proof: &C6AuthenticatedOutputLinkProof,
    nbr2: &C6Nbr2CorrectionFunctional<'_>,
    ...
    C6BoundSlotRegistryVerifier,
    C6Nbr2VerifiedLink,
    C63ResidualSourceFunctionalsVerifiedLink,
)> {
    refuse_production_reference(fixed)?;
    let bound = verify_c6_authenticated_output_link_inner(
        fixed,
        pending,
        proof,
        Some(nbr2),
    ...
    Ok((
        bound,
        C6Nbr2VerifiedLink { statement_digest: nbr2.digest() },
        C63ResidualSourceFunctionalsVerifiedLink {
            statement_digest: source_functionals.statement().digest(),
            terminal_m_keys: [source_functionals.keys()[0][0], source_functionals.keys()[1][1]],
        },
    ))
}
...
pub fn verify_c6_authenticated_output_link_production_nbr2(
    fixed: &C6FixedWrapperCommitments,
    pending: C6PendingSlotRegistryVerifier,
    proof: &C6AuthenticatedOutputLinkProof,
    nbr2: &C6Nbr2CorrectionFunctional<'_>,
...
pub fn verify_c63_authenticated_output_link_production_nbr2_strict(
    fixed: &C6FixedWrapperCommitments,
    pending: C6PendingSlotRegistryVerifier,
    proof: &C6AuthenticatedOutputLinkProof,
    nbr2: &C6Nbr2CorrectionFunctional<'_>,
    ...
    if !fixed.is_c63_authenticated_sketch_profile() {
        return Err(C6AuthenticatedOutputLinkError::new(
            "C6.3 production verifier requires authenticated-sketch roots",
        ));
    }
    let bound = verify_c6_authenticated_output_link_inner(
        fixed,
...
fn verify_c6_authenticated_output_link_inner(
    ...
    proof: &C6AuthenticatedOutputLinkProof,
    nbr2: Option<&C6Nbr2CorrectionFunctional<'_>>,
    c63_source: Option<&C63ResidualSourceFunctionalVerifierClaims>,
    contexts: &mut [VerifierCtx; C6_AUTHENTICATED_OUTPUT_LINK_TAPES],
    transcript: &mut Transcript,
) -> Result<C6BoundSlotRegistryVerifier> {
    let (relations, rounds, cohorts) = link_geometry(fixed)?;
    ...
    for repetition in 0..C6_WRAPPER_REPETITIONS {
        ...
        expected_schedule_digests.push(match c63_source {
            Some(source) => c63_source_functional_schedule_digest(
                schedule,
                source.statement().digest(),
                relations + usize::from(nbr2.is_some()) + 2,
    ...
    let mut reductions = Vec::with_capacity(C6_WRAPPER_REPETITIONS);
        ...
        proof.repetitions.iter().zip(&mut final_keys).enumerate()
    {
        ...
        let additional =
            link_additional_weights(beta, entries.len(), nbr2.is_some(), c63_source.is_some());
        let mut initial_keys = array::from_fn(|tape| {
            entries
                .iter()
                .zip(&rhos)
        ...
        if let Some(nbr2) = nbr2 {
            for tape in 0..C6_AUTHENTICATED_OUTPUT_LINK_TAPES {
                initial_keys[tape] = initial_keys[tape].add(
                    VerifierKey::from_public(nbr2.correction(), contexts[tape].delta)
                        .scale(additional.nbr2.expect("C6NBR2 weight")),
                );
            }
        }
        if let Some(source) = c63_source {
                ...
                additional.c63.expect("C6.3 source-functional weights").iter().enumerate()
            {
                for tape in 0..C6_AUTHENTICATED_OUTPUT_LINK_TAPES {
                    initial_keys[tape] =
                        initial_keys[tape].add(source.keys()[coordinate][tape].scale(weight));
                }
            }
        }
        ...
        reductions.push((descriptors, rhos, point, additional));
        *final_key_slot = keys;
    }
    let nbr2_evaluations = match nbr2 {
        Some(nbr2) => {
            let points = [&reductions[0].2[1..rounds - 1], &reductions[1].2[1..rounds - 1]];
            Some(
                evaluate_c6_nbr2_coefficient_prefix_at_two_points(nbr2.coefficients(), points)
                    .map_err(|error| {
                        C6AuthenticatedOutputLinkError::new(format!(
                            "C6NBR2 verifier functional evaluation: {error}"
                        ))
                    })?,
            )
        }
    ...
    let c63_evaluations = match c63_source {
        Some(source) => {
            let points = [&reductions[0].2[1..rounds - 1], &reductions[1].2[1..rounds - 1]];
            ...
            for coordinate in 0..2 {
                evaluations.push(
                    ...
                    .map_err(|error| {
                        C6AuthenticatedOutputLinkError::new(format!(
                            "C6.3 verifier source-functional evaluation: {error}"
                        ))
                    })?,
                );
            }
            Some([evaluations[0], evaluations[1]])
        }
        None => None,
    };
    let mut assembled_claims = Vec::with_capacity(C6_WRAPPER_REPETITIONS);
    for (repetition, (descriptors, rhos, point, additional)) in reductions.iter().enumerate() {
        let mut residual_slot_weights = Vec::with_capacity(3);
        if let (Some(evaluations), Some(gamma)) = (nbr2_evaluations, additional.nbr2) {
            residual_slot_weights.push((
                C6_NBR2_CORRECTION_SLOT,
                gamma
                    * (Fp2::ONE - point[0])
                    * (Fp2::ONE - point[point.len() - 1])
                    * evaluations[repetition],
            ));
        }
        if let (Some(evaluations), Some(gammas)) = (c63_evaluations, additional.c63) {
            for coordinate in 0..2 {
                residual_slot_weights.push((
                    C63_SOURCE_FUNCTIONAL_SLOTS[coordinate],
                    gammas[coordinate]
                        * (Fp2::ONE - point[0])
                        * (Fp2::ONE - point[point.len() - 1])
                        * evaluations[coordinate][repetition],
                ));
            }
        }
...
#[allow(dead_code)]
fn validate_persisted_prover_cohorts(
    fixed: &C6FixedWrapperCommitments,
    cohorts: &[C6PersistedWrapperCohort],
    session_digest: C6WrapperDigest,
) -> Result<()> {
    if session_digest == [0; 32]
        || cohorts.len() != fixed.commitments().len()
        || cohorts.iter().enumerate().zip(fixed.commitments()).any(
            |((ordinal, cohort), commitment)| {
                cohort.commitment() != commitment
                    || cohort.session_digest() != session_digest
                    || cohort.oracle_ordinal() != ordinal as u64
            },
        )
    {
        return Err(C6AuthenticatedOutputLinkError::new(
            "C6 persisted link prover cohorts do not match fixed roots/session",
        ));
    }
    Ok(())
}
...
#[derive(Clone, Copy)]
struct C6LinkAdditionalWeights {
    nbr2: Option<Fp2>,
    c63: Option<[Fp2; 2]>,
}

fn link_additional_weights(
    beta: Fp2,
    base_relations: usize,
    has_nbr2: bool,
    has_c63: bool,
) -> C6LinkAdditionalWeights {
    let mut next = (0..=base_relations).fold(Fp2::ONE, |power, _| power * beta);
    let nbr2 = has_nbr2.then(|| {
        let weight = next;
        next = next * beta;
        weight
    });
    let c63 = has_c63.then(|| {
        let first = next;
        let second = first * beta;
        [first, second]
    });
    C6LinkAdditionalWeights { nbr2, c63 }
}

fn prove_dual_tape_rounds(
    mut terms: Vec<C6ReferenceDelayedTerm>,
...
#[allow(clippy::too_many_arguments)]
#[allow(dead_code)]
fn prove_persisted_dual_tape_rounds(
    mut terms: Vec<C6PersistedCoefficientDelayedTerm>,
    mut claims: [ProverAuthed; C6_AUTHENTICATED_OUTPUT_LINK_TAPES],
    repetition: u8,
    streams: &mut [CorrelationStream; C6_AUTHENTICATED_OUTPUT_LINK_TAPES],
    transcript: &mut Transcript,
    rounds: usize,
    metrics: &mut C6PersistedLinkFoldMetrics,
) -> Result<C6PersistedLinkRoundOutput> {
    let mut point = Vec::with_capacity(rounds);
    let mut corrections = Vec::with_capacity(rounds);
    for round in 0..rounds {
        let (at_zero, at_two) =
            terms.iter().try_fold((Fp2::ZERO, Fp2::ZERO), |(zero, two), term| {
                let (term_zero, term_two) = term.round_values(metrics)?;
                Ok::<_, C6AuthenticatedOutputLinkError>((zero + term_zero, two + term_two))
            })?;
        let mut round_corrections = [[Fp2::ZERO; 2]; C6_AUTHENTICATED_OUTPUT_LINK_TAPES];
        let mut auth_zero = [ProverAuthed::ZERO; C6_AUTHENTICATED_OUTPUT_LINK_TAPES];
    ...
    for term in &terms {
        terminals.push(term.terminal(metrics)?);
    }
    let terminal = terminals.iter().copied().fold(Fp2::ZERO, |sum, value| sum + value);
    if claims.iter().any(|claim| claim.x != terminal) {
        return Err(C6AuthenticatedOutputLinkError::new(
            "C6 persisted link sumcheck terminal mismatch",
        ));
    }
    for term in terms {
        term.release(metrics)?;
    }
    Ok(C6PersistedLinkRoundOutput { point, final_claims: claims, corrections, terminals })
}

#[allow(clippy::too_many_arguments)]
fn prove_cuda_persisted_dual_tape_rounds(
    mut terms: Vec<C6CudaPersistedDelayedTerm>,
    mut claims: [ProverAuthed; C6_AUTHENTICATED_OUTPUT_LINK_TAPES],
    repetition: u8,
    streams: &mut [CorrelationStream; C6_AUTHENTICATED_OUTPUT_LINK_TAPES],
    transcript: &mut Transcript,
    ...
    for round in 0..rounds {
        ...
        let mut at_two = Fp2::ZERO;
        for term in &terms {
            let (term_zero, term_two) = term.round_values(backend, metrics)?;
            at_zero += term_zero;
            at_two += term_two;
        }
        let mut round_corrections = [[Fp2::ZERO; 2]; C6_AUTHENTICATED_OUTPUT_LINK_TAPES];
        ...
        for term in &mut terms {
            term.bind(backend, challenge, metrics)?;
        }
        point.push(challenge);
        corrections.push(round_corrections);
    }
    let mut terminals = Vec::with_capacity(terms.len());
    for term in &terms {
        terminals.push(term.terminal(backend, metrics)?);
    }
    let terminal = terminals.iter().copied().fold(Fp2::ZERO, |sum, value| sum + value);
    if claims.iter().any(|claim| claim.x != terminal) {
        return Err(C6AuthenticatedOutputLinkError::new(
            "C6 CUDA persisted link sumcheck terminal mismatch",
        ));
    }
    for term in terms {
        term.release(backend, metrics)?;
    }
    Ok(C6PersistedLinkRoundOutput { point, final_claims: claims, corrections, terminals })
}
...
fn assemble_new_point_claims<D: DescriptorView>(
    ...
    residual_slot_weight_additions: &[(u16, Fp2)],
    ...
    if supplied_aggregates.is_some_and(|aggregates| aggregates.len() != fixed.commitments().len()) {
        return Err(C6AuthenticatedOutputLinkError::new(
    ...
    for &(slot, addition) in residual_slot_weight_additions {
        let key = SlotKey { repetition, cohort_id: C6_DELTA_RESIDUAL_COHORT_ID, slot };
        let weight = weights.get_mut(&key).ok_or_else(|| {
            C6AuthenticatedOutputLinkError::new(
                "missing C6 authenticated residual-functional slot weight",
            )
        })?;
        *weight += addition;
    }
...
fn c63_source_functional_correlation_domain(coordinate: usize, tape: usize) -> Result<u64> {
    if coordinate >= 2 || tape >= C6_AUTHENTICATED_OUTPUT_LINK_TAPES {
        return Err(C6AuthenticatedOutputLinkError::new(
            "C6.3 source-functional correlation component is invalid",
        ));
    }
    let domain = C63_SOURCE_FUNCTIONAL_CORRELATION_BASE | ((tape as u64) << 24) | coordinate as u64;
    if domain & RESERVED_DOMAIN_BITS != 0 {
        return Err(C6AuthenticatedOutputLinkError::new(
            "C6.3 source-functional correlation domain uses reserved bits",
        ));
    }
    Ok(domain)
}
...
fn c63_source_functional_schedule_digest(
    base_schedule_digest: C6WrapperDigest,
    source_functional_digest: C6WrapperDigest,
    relations: usize,
) -> C6WrapperDigest {
    let mut hasher = blake3::Hasher::new_derive_key(C63_SOURCE_FUNCTIONAL_SCHEDULE_CONTEXT);
    hasher.update(&base_schedule_digest);
    hasher.update(&source_functional_digest);
    hasher.update(&(relations as u16).to_le_bytes());
    *hasher.finalize().as_bytes()
}

fn is_nbr2_correction_slot(descriptor: &C6PendingSlotDescriptor) -> bool {
    descriptor.cohort_id == C6_DELTA_RESIDUAL_COHORT_ID
        && descriptor.slot == C6_NBR2_CORRECTION_SLOT
}

fn residual_functional_terminal_weight(
    coefficients: &[Fp2],
    gamma: Fp2,
    point: &[Fp2],
) -> Result<Fp2> {
    if point.len() < 3 {
        return Err(C6AuthenticatedOutputLinkError::new(
            "C6 residual-functional terminal point is too short",
        ));
    }
    let source_point = &point[1..point.len() - 1];
    let capacity = 1usize.checked_shl(source_point.len() as u32).unwrap_or_default();
    if coefficients.len() > capacity {
        return Err(C6AuthenticatedOutputLinkError::new(
            "C6 residual functional exceeds terminal source domain",
        ));
    }
    let mut padded = coefficients.to_vec();
    padded.resize(capacity, Fp2::ZERO);
    let functional = evaluate_multilinear_table(&padded, source_point).map_err(|error| {
        C6AuthenticatedOutputLinkError::new(format!(
            "C6 residual-functional terminal evaluation: {error:?}"
        ))
    })?;
    Ok(gamma * (Fp2::ONE - point[0]) * (Fp2::ONE - point[point.len() - 1]) * functional)
}
...
mod tests {
    ...
    use crate::c6_wrapper_pcs::{
        ...
        prove_c6_wrapper_pcs_persisted_reference_assembled, C6CacheStateDescriptors,
        C6WrapperCohortSpec, C6WrapperCommitment, C6WrapperOracleKind, C6WrapperSlotWitness,
        C6_HIDDEN_U_EMBED_COHORT_ID, C6_HIDDEN_U_WEIGHTS_COHORT_ID, C6_PREDECESSOR_CACHE_COHORT_ID,
        C6_SUCCESSOR_CACHE_COHORT_ID,
    };
    ...
    fn c61_native_link_codec_is_exactly_56_plus_one_without_hidden_builder_api() {
        let cache_descriptors =
            C6CacheStateDescriptors::from_slots(std::array::from_fn(|index| [index as u8 + 1; 32]))
                .unwrap();
        ...
        let commitments = specs
            ...
            .map(|(index, spec)| {
                ...
                if index < 2 {
                    C6WrapperCommitment::from_cache_root([0x61; 32], spec, root, &cache_descriptors)
                        .unwrap()
                } else {
        ...
        assert!(C61NativePendingSlotRegistryProverBuilder::new(&fixed).unwrap().finish().is_err());
    ...
    fn nbr2_combined_term_preserves_degree_two_rounds_and_exact_terminal() {
        ...
        let zk_values = table(source_rounds, 41_500);
        let mut committed_values = values.clone();
        committed_values.extend_from_slice(&zk_values);
        ...
        let mut full_point = source_point.to_vec();
        full_point.push(challenges[source_rounds + 1]);
        let value_at_point = evaluate_multilinear_table(&committed_values, &full_point).unwrap();
    ...
    #[test]
    fn cuda_persisted_coefficient_term_matches_resident_without_host_arithmetic() {
        let mut backend = match Backend::cuda_resident() {
            Ok(backend) => backend,
            Err(_error) if std::env::var("VOLTA_REQUIRE_CUDA").as_deref() != Ok("1") => return,
            Err(error) => panic!("VOLTA_REQUIRE_CUDA set but C6LNK2 CUDA failed: {error}"),
        };
        let root = std::env::temp_dir().join(format!(
            "volta-c6-link-cuda-term-{}-{}",
            std::process::id(),
            std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_nanos()
        ));
        let cohort_root = root.join("cohorts");
        ...
        let evaluations = table(5, 29_500);
        let slots = (0..usize::from(spec.slot_count))
            .map(|slot| C6WrapperSlotWitness::Auxiliary {
                evaluations: if slot == 0 || slot == usize::from(C6_NBR2_CORRECTION_SLOT) {
                    evaluations.clone()
                } else {
                    table(5, 30_000 + slot as u64 * 100)
                },
            })
            .collect::<Vec<_>>();
        let cohort = commit_c6_wrapper_cohort(statement_digest, spec, slots).unwrap();
        let persisted =
            persist_scaled_c6_wrapper_cohort_reference(cohort, &cohort_root, [0x72; 32], 0)
                .unwrap();
        let source = persisted.open_coefficient_slots().unwrap();
        let target = (0..5).map(|index| symbol(30_500 + index)).collect::<Vec<_>>();
        let descriptor = C6PendingSlotDescriptor {
            wrapper_statement_digest: statement_digest,
            fixed_roots_digest: [0x73; 32],
            source_statement_digest: [0x74; 32],
            repetition: 0,
            cohort_id: spec.cohort_id,
            slot: 0,
            target_point: target.clone(),
        };
        let coefficient = symbol(91);
        let claimed_value = evaluate_multilinear_table(&evaluations, &target).unwrap();
        let mut resident = DelayedTerm::new(coefficient, &evaluations, &target, 7).unwrap();
        let mut metrics = C6PersistedLinkFoldMetrics::default();
        let mut cuda_term = C6CudaPersistedCoefficientDelayedTerm::new(
            coefficient,
            claimed_value,
            &descriptor,
            &source,
        ...
        .unwrap();
        assert_eq!(cuda_term.initial_value(), resident.active_sum());
        for round in 0..7 {
            assert_eq!(
                cuda_term.round_values(&mut backend, &mut metrics).unwrap(),
                resident.round_values().unwrap(),
                "round {round}",
            );
            let challenge = symbol(31_500 + round as u64);
            resident.bind(challenge);
            cuda_term.bind(&mut backend, challenge, &mut metrics).unwrap();
        }
        assert_eq!(
            cuda_term.terminal(&mut backend, &mut metrics).unwrap(),
            resident.terminal().unwrap()
        );
        cuda_term.release(&mut backend, &mut metrics).unwrap();
        assert_eq!(metrics.coefficient_bytes_read, 512);
        assert_eq!(metrics.coefficient_bytes_written, 496);
        assert_eq!(metrics.manifest_bytes_written, 1_792);
        assert_eq!(metrics.files_created, 12);
        assert_eq!(metrics.files_deleted_after_successor_durable, 12);
        ...
        assert_eq!(metrics.current_live_spill_bytes, 0);
        assert_eq!(metrics.peak_live_spill_bytes, 896);
        assert_eq!(metrics.cuda_h2d_bytes, 1_632);
        assert_eq!(metrics.cuda_d2h_bytes, 672);
        assert_eq!(metrics.cuda_kernel_calls, 35);

        let nbr2_spill_root = root.join("nbr2-link");
        std::fs::create_dir(&nbr2_spill_root).unwrap();
        let mut nbr2_target = (0..4).map(|index| symbol(32_500 + index)).collect::<Vec<_>>();
        nbr2_target.push(Fp2::ZERO);
        let nbr2_descriptor = C6PendingSlotDescriptor {
            wrapper_statement_digest: statement_digest,
            fixed_roots_digest: [0x73; 32],
            source_statement_digest: [0x74; 32],
            repetition: 0,
            cohort_id: C6_DELTA_RESIDUAL_COHORT_ID,
            slot: C6_NBR2_CORRECTION_SLOT,
            target_point: nbr2_target.clone(),
        };
        let functional = (0..13).map(|index| symbol(33_000 + index as u64)).collect::<Vec<_>>();
        let correction = evaluations[..16]
            .iter()
            .zip(&functional)
            .fold(Fp2::ZERO, |sum, (&value, &coefficient)| sum + value * coefficient);
        let mut nbr2 = C6Nbr2CorrectionFunctional {
            outer_statement_digest: [0x81; 32],
            wrapper_statement_digest: statement_digest,
            fixed_roots_digest: [0x73; 32],
            ...
            coefficient_vector_digest: c6_nbr2_coefficient_vector_digest(&functional),
            source_count: functional.len() as u32,
            source_log2: 4,
            correction,
            coefficients: &functional,
            digest: [0; 32],
        };
        nbr2.digest = c6_nbr2_statement_digest(&nbr2);
        let rho = symbol(34_000);
        let gamma = symbol(34_001);
        let nbr2_claimed = evaluate_multilinear_table(&evaluations, &nbr2_target).unwrap();
        let mut resident_nbr2 = C6Nbr2CombinedDelayedTerm::new(
            rho,
            gamma,
            &evaluations,
            &nbr2_target,
        ...
        .unwrap();
        let mut nbr2_metrics = C6PersistedLinkFoldMetrics::default();
        let mut cuda_nbr2 = C6CudaNbr2CombinedDelayedTerm::new(
            rho,
            gamma,
            nbr2_claimed,
            &nbr2_descriptor,
        ...
        .unwrap();
        assert_eq!(cuda_nbr2.initial_value(), resident_nbr2.active_sum());
        for round in 0..6 {
            assert_eq!(
                cuda_nbr2.round_values(&mut backend, &mut nbr2_metrics).unwrap(),
                resident_nbr2.round_values().unwrap(),
                "C6NBR2 round {round}",
            );
            let challenge = symbol(35_000 + round as u64);
            resident_nbr2.bind(challenge).unwrap();
            cuda_nbr2.bind(&mut backend, challenge, &mut nbr2_metrics).unwrap();
        }
        assert_eq!(
            cuda_nbr2.terminal(&mut backend, &mut nbr2_metrics).unwrap(),
            resident_nbr2.terminal().unwrap(),
        );
        cuda_nbr2.release(&mut backend, &mut nbr2_metrics).unwrap();
        assert_eq!(nbr2_metrics.current_live_spill_bytes, 0);
        drop(source);
        drop(persisted);
        std::fs::remove_dir_all(root).unwrap();
    }
    ...
    #[test]
    fn all_72_persisted_terms_match_resident_bytes_transcript_correlations_and_terminals() {
        let reference = prove_integrated_fixture();
        ...
        let pcs_root = root.join("pcs");
        std::fs::create_dir(&root).unwrap();
        std::fs::create_dir(&cohort_root).unwrap();
        std::fs::create_dir(&link_root).unwrap();
        std::fs::create_dir(&pcs_root).unwrap();
        ...
        let fixed =
            fix_test_c6_wrapper_commitments([0xC6; 32], &inputs.commitments, &mut transcript)
                .unwrap();
        let (_, _, residual_pending) = prove_c6_blind_residual_sumchecks_reference(
            &inputs.statements,
            &inputs.witnesses,
            &mut streams,
            &mut transcript,
        ...
        .unwrap();
        let (_, hidden_pending) = prove_c6_blind_hidden_u_sumchecks_reference(
        ...
        let (_, _, cache_pending, _) = prove_c6_persistent_cache_blind_reference(
            &cache_plan,
        ...
        assert_eq!(pending.len(), 2 * C6_AUTHENTICATED_OUTPUT_LINK_PRODUCTION_RELATIONS);
        let before: [u64; C6_AUTHENTICATED_OUTPUT_LINK_TAPES] =
            array::from_fn(|tape| streams[tape].counters.full_corrs);
        let session_digest = [0x91; 32];
        let persisted_cohorts = inputs
            .cohorts
            .into_iter()
            .enumerate()
            .map(|(ordinal, cohort)| {
                persist_scaled_c6_wrapper_cohort_reference(
                    cohort,
                    &cohort_root,
                    session_digest,
                    ordinal as u64,
            ...
            .collect::<Vec<_>>();
        let preparation = prepare_c6_authenticated_output_link_persisted(
            &fixed,
            &persisted_cohorts,
            &pending,
            &mut streams,
        ...
        .unwrap();
        let C6PersistedLinkPreparation { repetition_proofs, assembled, final_claims, metrics } =
            preparation;
        let wrapper_pcs = prove_c6_wrapper_pcs_persisted_reference_assembled(
            fixed.statement_digest(),
            &persisted_cohorts,
            &assembled,
            &pcs_root,
        ...
        for repetition in 0..C6_WRAPPER_REPETITIONS {
            ...
            for tape in 0..C6_AUTHENTICATED_OUTPUT_LINK_TAPES {
                let residual =
                    final_claims[repetition][tape].sub(ProverAuthed::from_public(opened));
                assert_eq!(residual.x, Fp2::ZERO);
                terminal_tags[repetition][tape] = zero_open_prover(&residual, &mut transcript);
            }
        }
        let proof = C6AuthenticatedOutputLinkProof {
            repetitions: repetition_proofs,
        ...
        transcript.append(LINK_DIGEST_LABEL, LINK_DIGEST_BYTES);
        assert_eq!(proof.canonical_bytes(&fixed).unwrap(), reference.encoded);
        assert_eq!(transcript.ledger(), &reference.prover_ledger);
        assert_eq!(transcript.total_bytes(), reference.prover_total);
        assert_eq!(
            array::from_fn::<_, C6_AUTHENTICATED_OUTPUT_LINK_TAPES, _>(|tape| {
                streams[tape].counters.full_corrs - before[tape]
            }),
            reference.prover_link_counter_delta
        );
        assert_eq!(metrics.coefficient_bytes_read, 566_528);
        assert_eq!(metrics.coefficient_bytes_written, 112_384);
        assert_eq!(metrics.manifest_bytes_written, 258_048);
        assert_eq!(metrics.files_created, 1_744);
        assert_eq!(metrics.files_deleted_after_successor_durable, 1_744);
        assert_eq!(metrics.directories_created, 144);
        assert_eq!(metrics.fsync_count, 3_904);
        assert_eq!(metrics.current_live_spill_bytes, 0);
        assert_eq!(metrics.peak_live_spill_bytes, 35_584);
        drop(persisted_cohorts);
        std::fs::remove_dir_all(root).unwrap();
    }

    #[test]
    fn production_persisted_link_refuses_cpu_before_allocation_io_or_transcript() {
        let inputs = scaled_inputs();
        let mut transcript = Transcript::new(CHALLENGE_SEED);
        let fixed =
            fix_test_c6_wrapper_commitments([0xC6; 32], &inputs.commitments, &mut transcript)
                .unwrap();
        let pending = C6PendingSlotRegistryProver {
            wrapper_statement_digest: fixed.statement_digest(),
            fixed_roots_digest: fixed.binding_digest(),
            entries: Vec::new(),
        };
        let root = std::env::temp_dir().join(format!(
            "volta-c6-link-cpu-reject-{}-{}",
            std::process::id(),
            std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_nanos()
        ));
        let before_ledger = transcript.ledger().clone();
        let before_total = transcript.total_bytes();
        let mut streams = array::from_fn(|tape| CorrelationStream::new(TAPE_SEEDS[tape]));
        let before_correlations =
            array::from_fn::<_, C6_AUTHENTICATED_OUTPUT_LINK_TAPES, _>(|tape| {
                streams[tape].counters.full_corrs
            });
        let error = prove_c6_authenticated_output_link_persisted_cuda(
            &fixed,
            &[],
            pending,
            &mut streams,
        ...
        .unwrap_err();
        assert!(error.to_string().contains("requires cuda-resident backend"));
        assert!(!root.exists());
        assert_eq!(transcript.ledger(), &before_ledger);
        assert_eq!(transcript.total_bytes(), before_total);
        assert_eq!(
            array::from_fn::<_, C6_AUTHENTICATED_OUTPUT_LINK_TAPES, _>(|tape| {
                streams[tape].counters.full_corrs
            }),
    ...
    struct IntegratedFixture {
        ...
        nbr2: Option<Nbr2Fixture>,
        c63_source: Option<C63SourceFixture>,
    }
    ...
    struct C63SourceFixture {
        coefficients: [Vec<Fp2>; 2],
        frame: C63ResidualSourceFunctionalFrame,
    }

    impl C63SourceFixture {
        fn statement(&self, fixed: &C6FixedWrapperCommitments) -> C63ResidualSourceFunctionals {
            C63ResidualSourceFunctionals::new_owned(
                fixed,
                [0xa1; 32],
                [0xa2; 32],
                [0xa3; 32],
    ...
    impl Nbr2Fixture {
        fn statement<'a>(
            &'a self,
            fixed: &C6FixedWrapperCommitments,
        ) -> C6Nbr2CorrectionFunctional<'a> {
            C6Nbr2CorrectionFunctional::new(
                fixed,
                [0x91; 32],
                [0x92; 32],
                [0x93; 32],
    ...
    fn prove_integrated_fixture() -> IntegratedFixture {
        let inputs = scaled_inputs();
        prove_integrated_fixture_with(inputs, false, false, |inputs, streams, transcript| {
    ...
    fn prove_nbr2_integrated_fixture() -> IntegratedFixture {
        let inputs = scaled_inputs();
        prove_integrated_fixture_with(inputs, true, false, |inputs, streams, transcript| {
            prove_c6_blind_residual_sumchecks_reference(
                &inputs.statements,
                &inputs.witnesses,
                streams,
    ...
    fn prove_c63_source_integrated_fixture() -> IntegratedFixture {
        let inputs = scaled_inputs();
        prove_integrated_fixture_with(inputs, true, true, |inputs, streams, transcript| {
            prove_c6_blind_residual_sumchecks_reference(
                &inputs.statements,
                &inputs.witnesses,
                streams,
    ...
    fn prove_integrated_fixture_with(
        ...
        assert!(!with_c63_source || with_nbr2);
        let mut streams = array::from_fn(|tape| CorrelationStream::new(TAPE_SEEDS[tape]));
        let mut transcript = Transcript::new(CHALLENGE_SEED);
        let fixed =
            fix_test_c6_wrapper_commitments([0xC6; 32], &inputs.commitments, &mut transcript)
        ...
        let nbr2 = with_nbr2.then(|| {
            let residual = &inputs.tables[&(C6_DELTA_RESIDUAL_COHORT_ID, C6_NBR2_CORRECTION_SLOT)];
            let lower = &residual[..residual.len() / 2];
            let coefficients = (0..lower.len() - 3)
                .map(|index| symbol(880_000 + index as u64))
                .collect::<Vec<_>>();
            let correction = lower
                .iter()
                .zip(&coefficients)
                .fold(Fp2::ZERO, |sum, (&value, &coefficient)| sum + value * coefficient);
            Nbr2Fixture { coefficients, correction }
        });
        let mut c63_source = with_c63_source.then(|| {
            let coefficients = C63_SOURCE_FUNCTIONAL_SLOTS.map(|slot| {
                let residual = &inputs.tables[&(C6_DELTA_RESIDUAL_COHORT_ID, slot)];
                (0..residual.len() / 2 - 3)
                    .map(|index| symbol(890_000 + u64::from(slot) * 10_000 + index as u64))
                    .collect::<Vec<_>>()
            });
            C63SourceFixture {
                coefficients,
        ...
        let before: [u64; C6_AUTHENTICATED_OUTPUT_LINK_TAPES] =
            array::from_fn(|tape| streams[tape].counters.full_corrs);
        let (proof, bound, metrics) = if let Some(source) = &c63_source {
            let values = array::from_fn(|coordinate| {
                let slot = C63_SOURCE_FUNCTIONAL_SLOTS[coordinate];
                let residual = &inputs.tables[&(C6_DELTA_RESIDUAL_COHORT_ID, slot)];
                residual[..residual.len() / 2]
                    .iter()
                    .zip(&source.coefficients[coordinate])
                    .fold(Fp2::ZERO, |sum, (&value, &coefficient)| sum + value * coefficient)
            });
            let (claims, frame) = authenticate_c63_residual_source_functionals(
                source.statement(&fixed),
                values,
                &mut streams,
                &mut transcript,
            )
            .unwrap();
            let (proof, bound, metrics, _, _) =
                prove_c63_authenticated_output_link_reference_nbr2_strict(
                    &fixed,
                    &inputs.cohorts,
                    pending,
                    &polynomial_views(&inputs),
                    &nbr2.as_ref().unwrap().statement(&fixed),
                    &claims,
                    &mut streams,
                    &mut transcript,
                )
                .unwrap();
            c63_source.as_mut().unwrap().frame = frame;
            (proof, bound, metrics)
        } else {
            match &nbr2 {
                Some(nbr2) => prove_c6_authenticated_output_link_reference_nbr2(
                    &fixed,
                    &inputs.cohorts,
                    pending,
                    &polynomial_views(&inputs),
                    &nbr2.statement(&fixed),
                    &mut streams,
                    &mut transcript,
                ),
                None => prove_c6_authenticated_output_link_reference(
                    &fixed,
                    &inputs.cohorts,
                    pending,
                    &polynomial_views(&inputs),
    ...
    #[test]
    fn nbr2_fuses_the_exact_correction_without_wire_or_correlation_growth() {
        let baseline = prove_integrated_fixture();
        let fixture = prove_nbr2_integrated_fixture();
        assert_eq!(fixture.encoded.len(), baseline.encoded.len());
        assert_eq!(fixture.metrics.combined_proof_bytes, baseline.metrics.combined_proof_bytes);
        assert_eq!(fixture.metrics.link_overhead_bytes, baseline.metrics.link_overhead_bytes);
        assert_eq!(fixture.metrics.relations_per_repetition, 73);
        assert_eq!(fixture.prover_link_counter_delta, baseline.prover_link_counter_delta);
        assert_eq!(fixture.prover_total, baseline.prover_total);
        assert_eq!(fixture.prover_ledger.get(LINK_NBR2_BOUND_LABEL), Some(&0));
        ...
        let mut changed_coefficients = fixture.nbr2.as_ref().unwrap().coefficients.clone();
        changed_coefficients[0] += Fp2::ONE;
        let (fixed, pending, mut contexts, mut transcript, _) = verifier_prefix(&fixture);
        let changed = C6Nbr2CorrectionFunctional::new(
            &fixed,
            [0x91; 32],
            [0x92; 32],
            [0x93; 32],
            ...
            fixture.nbr2.as_ref().unwrap().correction,
        )
        .unwrap();
        ...
        let (fixed, pending, mut contexts, mut transcript, _) = verifier_prefix(&fixture);
        let original = fixture.nbr2.as_ref().unwrap();
        let changed_correction = C6Nbr2CorrectionFunctional::new(
            &fixed,
            [0x91; 32],
            [0x92; 32],
            [0x93; 32],
        ...
        let mut changed_round = fixture.proof.clone();
        changed_round.repetitions[1].corrections[2][0][0] += Fp2::ONE;
        let (fixed, pending, mut contexts, mut transcript, _) = verifier_prefix(&fixture);
        let nbr2 = fixture.nbr2.as_ref().unwrap().statement(&fixed);
        assert!(verify_c6_authenticated_output_link_reference_nbr2(
            &fixed,
    ...
    #[test]
    fn c63_private_source_functionals_share_the_link_and_reject_drift() {
        let baseline = prove_nbr2_integrated_fixture();
        let fixture = prove_c63_source_integrated_fixture();
        let source = fixture.c63_source.as_ref().unwrap();
        assert_eq!(fixture.encoded.len(), baseline.encoded.len());
        assert_eq!(fixture.metrics.combined_proof_bytes, baseline.metrics.combined_proof_bytes);
        assert_eq!(fixture.metrics.relations_per_repetition, 75);
        assert_eq!(
            fixture.prover_link_counter_delta,
            baseline.prover_link_counter_delta.map(|count| count + 2)
        );
        assert_eq!(fixture.prover_total, baseline.prover_total + 64);
        assert_eq!(fixture.prover_ledger.get(C63_SOURCE_FUNCTIONAL_CORRECTIONS_LABEL), Some(&64));
        assert_eq!(fixture.prover_ledger.get(LINK_C63_SOURCE_BOUND_LABEL), Some(&0));
        assert_eq!(
            C63ResidualSourceFunctionalFrame::decode(&source.frame.encode()).unwrap(),
            source.frame
        );

        let (fixed, pending, mut contexts, mut transcript, before) = verifier_prefix(&fixture);
        let source_claims = verify_c63_residual_source_functionals(
            source.statement(&fixed),
            &source.frame,
            &mut contexts,
            &mut transcript,
        ...
        .unwrap();
        let nbr2 = fixture.nbr2.as_ref().unwrap().statement(&fixed);
        let (bound, nbr2_receipt, source_receipt) =
            verify_c63_authenticated_output_link_reference_nbr2_strict(
                &fixed,
                pending,
                &fixture.proof,
                &nbr2,
            ...
            .unwrap();
        assert_eq!(bound.len(), 2 * C6_WRAPPER_ACTIVE_SLOTS);
        assert_eq!(nbr2_receipt.statement_digest(), nbr2.digest());
        assert_eq!(source_receipt.statement_digest(), source.statement(&fixed).digest());
        assert_eq!(transcript.total_bytes(), fixture.prover_total);
        assert_eq!(
            array::from_fn::<_, C6_AUTHENTICATED_OUTPUT_LINK_TAPES, _>(|tape| {
                contexts[tape].counters.full_corrs - before[tape]
        ...
        let mut changed_frame = source.frame;
        changed_frame.corrections[0][0] += Fp2::ONE;
        let (fixed, pending, mut contexts, mut transcript, _) = verifier_prefix(&fixture);
        let changed_claims = verify_c63_residual_source_functionals(
            source.statement(&fixed),
            &changed_frame,
            &mut contexts,
            &mut transcript,
        ...
        .unwrap();
        let nbr2 = fixture.nbr2.as_ref().unwrap().statement(&fixed);
        assert!(verify_c63_authenticated_output_link_reference_nbr2_strict(
            &fixed,
            pending,
            &fixture.proof,
            &nbr2,
        ...
        let mut changed_coefficients = source.coefficients.clone();
        changed_coefficients[1][0] += Fp2::ONE;
        let (fixed, pending, mut contexts, mut transcript, _) = verifier_prefix(&fixture);
        let changed_statement = C63ResidualSourceFunctionals::new(
            &fixed,
            [0xa1; 32],
            [0xa2; 32],
            [0xa3; 32],
            [0xa4; 32],
            [&changed_coefficients[0], &changed_coefficients[1]],
        )
        .unwrap();
        let changed_claims = verify_c63_residual_source_functionals(
            changed_statement,
