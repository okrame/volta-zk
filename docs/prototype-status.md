...
| C6 role-local instance extraction codec | **TWO-SEED DIAGNOSTIC PASS; CLIENT MAP FITS; RUNTIME RECORDER / CLEAN RECORDS PENDING** (2026-07-29) | No response-value vector on wire; role-specific model-global maps; maximal-`+1` run codec fixed before measurement; strict ordinary-build decoder | Both seed-24/25 runs reconstruct their distinct instance digests and emit stable role artifacts.  The verifier map is **5,320,386 B** with **1,466 / 10,828,876 raw public/scalar slots**, **1,436 / 10,828,852 canonical slots** and **262 / 2,552,791 runs**; BLAKE3 `17ed0942...7183535`.  Client topology + map are **69,315,137 B**; paired PCG + setup envelope total **146,058,504 B**, leaving **3,941,496 B**.  Dirty local diagnostics only; no runtime, prover-time, residual or production claim. |
...
- **2026-07-29 — C6 role-local instance extraction materializes and fits;
  ...
  **146,058,504 B <=150,000,000 B**, leaving **3,941,496 B**.  The strict
  ...
  runtime instance extraction.  Both measurements are dirty local
  implementation diagnostics; clean append-only records, the lightweight
  value recorder, provider wall, compiled residual, complete setup and
  production verdict remain pending.  No provider/pod was contacted.
