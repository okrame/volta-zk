...
| C6 installed compiled-residual core | **SCALED ALGEBRA / PAIRED-STREAM SEAM GREEN; FULL-T1 RECORD AND TIMING PENDING** (2026-07-29) | Consuming strict decoder installation; local reverse workspace; role-map runtime values; transcript-streamed base-share coefficients; paired source/key folds | The installed plan materializes typed opcode/source/operand/terminal arrays from the same strict decoder and never serializes them.  Scaled differential tests match the historical residual exactly, cover interleaved subfield/full-field/ProductMask leaves in both MAC coordinates, and reject transcript or source-schedule divergence.  The paired witness now binds the accepted `source_schedule_digest` in addition to its physical allocation digest.  The full `c6-trace` proto suite is **138 pass / 0 fail / 1 ignored**.  No full-T1 residual, installed-memory, ordinary overhead, response-byte, wrapper, real-PCG or hardware credit is claimed. |
